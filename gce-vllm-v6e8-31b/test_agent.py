...
class TestProvisioningModel(unittest.IsolatedAsyncioTestCase):
    ...
    def test_reservation_bound_targets_a_specific_reservation(self):
        ...
        flags = _provisioning_flags("reservation-bound", reservation_name="cal-v6e-8")
        self.assertIn("--reservation-affinity=specific", flags)
        self.assertIn("--reservation=cal-v6e-8", flags)
    ...
    def test_reservation_name_is_ignored_by_the_other_models(self):
        ...
        for model in ("flex-start", "spot", "on-demand"):
            flags = _provisioning_flags(model, reservation_name="cal-v6e-8")
    ...
    async def test_create_uses_screaming_case_provisioning_model(self, mock_run_command, mock_get_secret):
        ...
        self.assertIn("--machine-type=ct6e-standard-8t", cmd)
        self.assertNotIn("--accelerator-type=v6e-8", cmd)
...
class TestCostFromLivePricing(unittest.IsolatedAsyncioTestCase):
    ...
    def test_topology_parsing(self):
        ...
        self.assertIsNone(_parse_topology("v6e-8"))

    def test_chip_count_is_derived_from_the_machine_type(self):
        """A literal chip count and the machine type must never be able to disagree.

        Quota is metered in chips and every catalog rate is per chip-hour, so a stale 1 here
        would understate both by a factor of eight — and understate them silently, since a
        create with too little quota queues rather than erroring.
        """
        self.assertEqual(server._chips_in_machine_type("ct6e-standard-8t"), 8)
        self.assertEqual(server._chips_in_machine_type("ct6e-standard-8t-tpu"), 8)
        self.assertEqual(server._chips_in_machine_type("ct6e-standard-1t"), 1)
        self.assertEqual(server._chips_in_machine_type("ct6e-standard-4t"), 4)
        self.assertEqual(server._chips_in_machine_type("ct5p-hightpu-4t"), 4)
        self.assertIsNone(server._chips_in_machine_type("n2-standard-8"))

    def test_this_rig_is_eight_chips_and_its_defaults_agree(self):
        """The three ways this rig states its size must all say eight."""
        self.assertEqual(server.MACHINE_TYPE, "ct6e-standard-8t")
        self.assertEqual(server.CHIP_COUNT, 8)
        self.assertEqual(_parse_topology(server.TOPOLOGY), server.CHIP_COUNT)
...
def _node(name, ip="10.0.0.1", status="RUNNING", external=True, rig=None):
    ...
    return {
        ...
        "status": status,
        "machineType": "projects/p/zones/us-east5-b/machineTypes/ct6e-standard-8t",
...
class TestNodeDiscovery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_ct6e_instance_is_not_a_tpu_vm_node(self):
        ...
        async def _run(cmd, timeout=60):
            ...
            if "queued-resources" in cmd:
                return 0, "[]", ""
            if "instances" in cmd and "list" in cmd:
                return 0, json.dumps([_node("gce-vllm-v6e8-31b")]), ""
            return 1, "", "unexpected"

        with (
            patch("server.run_command", new=AsyncMock(side_effect=_run)),
            patch("server._probe_vllm", new=AsyncMock(return_value="google/gemma-4-31B-it")),
        ):
    ...
    async def test_no_remote_tool_reaches_the_vm_through_tpu_vm_ssh(self):
        ...
        with (
            patch("server.run_command", new=AsyncMock(side_effect=_run)),
            patch("server._resolve_node_id", new=AsyncMock(return_value="gce-vllm-v6e8-31b")),
        ):
            await server.manage_vllm_docker("gce-vllm-v6e8-31b", action="status")
            await server.get_vllm_docker_logs("gce-vllm-v6e8-31b")
            await server.get_tpu_system_logs("gce-vllm-v6e8-31b")
    ...
    async def test_docker_commands_install_docker_when_the_image_lacks_it(self):
        ...
        with (
            ...
            await server.manage_vllm_docker("gce-vllm-v6e8-31b", action="start")
            docker_cmd = seen[-1][-1]
            seen.clear()
            await server.get_tpu_system_logs("gce-vllm-v6e8-31b")
    ...
    async def test_this_rigs_instance_wins_over_a_siblings(self):
        ...
        with (
            ...
            patch("server._probe_vllm", new=AsyncMock(return_value="google/gemma-4-31B-it")) as probe,
        ):
            node = await _discover_vllm_node()
        assert node is not None
...
class TestSweepOverrides(unittest.IsolatedAsyncioTestCase):
    """The per-run flag overrides that make a TP sweep affordable on one instance.

    The whole point of the overrides is to re-serve ONE instance under several configs so
    the 62 GB checkpoint is pulled once. That only saves money if the override actually
    reaches vLLM, and the way it silently does not is `docker start`.
    """

    async def test_no_overrides_renders_exactly_what_it_always_did(self):
        """The boot path must be byte-identical unless a caller opts in."""
        flags = _vllm_serve_flags()
        self.assertIn(f"--tensor-parallel-size {server.TENSOR_PARALLEL_SIZE}", flags)
        self.assertIn(f"--max-model-len {server.MAX_MODEL_LEN}", flags)
        # Flags with no tpu.env default are omitted entirely rather than guessed at.
        self.assertNotIn("--kv-cache-dtype", flags)
        self.assertNotIn("--gpu-memory-utilization", flags)

    async def test_overrides_reach_the_flag_list(self):
        flags = _vllm_serve_flags(
            tensor_parallel_size=4, max_model_len=2048, kv_cache_dtype="fp8_e4m3", gpu_memory_utilization=0.85
        )
        self.assertIn("--tensor-parallel-size 4", flags)
        self.assertIn("--max-model-len 2048", flags)
        self.assertIn("--kv-cache-dtype fp8_e4m3", flags)
        self.assertIn("--gpu-memory-utilization 0.85", flags)
        self.assertNotIn(f"--tensor-parallel-size {server.TENSOR_PARALLEL_SIZE} ", flags)

    async def test_an_override_forces_a_rebuild_and_never_docker_start(self):
        """`docker start` replays the argv the container was BUILT with.

        Without the forced rm, a TP=4 sweep step against a container built at TP=8 would
        report success and serve at 8 — a plausible, wrong, and completely silent result.
        """
        captured = {}
        ...
        with (
            ...
            out = await manage_vllm_docker(action="start", tensor_parallel_size=4)

        self.assertIn("docker rm -f vllm-gemma4", captured["cmd"])
        self.assertNotIn("docker start vllm-gemma4", captured["cmd"])
        self.assertIn("--tensor-parallel-size 4", captured["cmd"])
        self.assertIn("REBUILT", out)

    async def test_without_overrides_start_still_reuses_the_container(self):
        """Cheap restart stays cheap when nothing changed."""
        captured = {}
        ...
        with (
            ...
            await manage_vllm_docker(action="start")
    ...
    async def test_overrides_are_refused_on_actions_that_cannot_apply_them(self):
        ...
        with patch("server._resolve_node_id", new=AsyncMock(return_value="node-1")):
            out = await manage_vllm_docker(action="log", tensor_parallel_size=4)
        self.assertIn("only apply to", out)


class TestStagedCheckpointRestore(unittest.IsolatedAsyncioTestCase):
    """MODEL_GCS_URI is an optimization and must never become a new failure mode."""

    def _render(self, uri):
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "startup_script_template.sh")) as f:
    ...
    def test_template_still_renders_with_and_without_a_staged_uri(self):
        """A stray brace in the new block would break the deploy at format() time."""
        for uri in ("", "gs://bucket/model.hfcache.tar"):
            self.assertIn("docker run --name vllm-gemma4", self._render(uri))

    def test_rendered_script_is_valid_bash_in_both_cases(self):
        for uri in ("", "gs://bucket/model.hfcache.tar"):
            with tempfile.NamedTemporaryFile("w", suffix=".sh", delete=False) as fh:
                fh.write(self._render(uri))
                path = fh.name
            try:
    ...
    def test_restore_falls_back_rather_than_failing_the_boot(self):
        script = self._render("gs://bucket/model.hfcache.tar")
        # Existence is checked before the stream, and every failure path says "falling back".
        self.assertIn("gcloud storage ls", script)
        self.assertIn("Falling back to the normal Hugging Face pull", script)

    def test_offline_is_only_set_after_a_restore_that_worked(self):
        """HF_HUB_OFFLINE on an incomplete cache turns a slow boot into a dead one."""
        script = self._render("gs://bucket/model.hfcache.tar")
        offline = script.index('HF_OFFLINE_FLAG="-e HF_HUB_OFFLINE=1"')
        guard = script.index("if [ $RESTORE_RC -eq 0 ]")
        self.assertLess(guard, offline)

    def test_the_cache_is_streamed_as_a_tar_not_copied_as_a_tree(self):
        """`gcloud storage cp -r` follows the HF cache's symlinks and doubles the transfer."""
        script = self._render("gs://bucket/model.hfcache.tar")

...
## Current Deployment
*   **Model:** `google/gemma-4-31B-it` on TPU v6e-8 (Trillium) — eight chips, `ct6e-standard-8t`.
*   **Endpoint:** discovered at runtime from the RUNNING Compute Engine instance — use the
    `get_vllm_endpoint` tool or `make endpoint`. Do not hardcode an IP. (This file is inherited and still
    says "Queued Resource" in places; `CLAUDE.md` is correct where they disagree.)
...
chip comparison (third-party, directional only — different model, different harness, and since this rig
moved to v6e-8, different hardware from what it benchmarks)
...
*   `MCP_SERVER_NAME`: Name this server advertises, and the key it must be registered under — it prefixes
    every tool as `mcp__<name>__find_tpu` (defaults to the rig directory name, `gce-vllm-v6e8-31b`).

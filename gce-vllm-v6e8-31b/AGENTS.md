...
> **This file is inherited and still describes the Queued Resource path. `CLAUDE.md` is correct where the
> two disagree.** The hardware facts below were corrected for the v6e-8 retarget on 2026-08-19; the
> control-plane prose was not.

This is a live-demo DevOps/SRE agent for serving `google/gemma-4-31B-it` with vLLM on a Google Cloud TPU
v6e-8 (Trillium) slice — eight chips on one `ct6e-standard-8t` Compute Engine instance. The main application is a single-file MCP server (`MCPServer`) in `server.py`. Its
...
- A v6e-8 is eight chips on one host, and the default tensor parallel size is `8`. That uses the slice but
  is required rather than chosen — 57.7 GiB of bf16 weights do not fit one 31.24 GiB chip. It shards the
  31B's 50 sliding layers exactly (16 KV heads) and pads its 10 full layers (4 heads → 8). See
  `CLAUDE.md` and `When_TP_Crosses_the_KV_Head_Count_v6e8.pdf`.
- gcloud spells v6e `v6e`, with none of the `v5litepod` renaming v5e has. The accelerator type is `v6e-8`,
  the Flex-start runtime version is `v2-alpha-tpuv6e`, and `--type=v6e --topology=2x4` is the tpu-vm form.
  On *this* rig none of those are used: it provisions `--machine-type=ct6e-standard-8t` through
  `gcloud compute instances`.
  The quota ids are `TPUV6EPerProjectPerZoneForTPUAPI` and `TPUV6EPreemptiblePerProjectPerZoneForTPUAPI`
  (no `Litepod`). Read them from `tpu.env` / `server.py` rather than from the directory name — `v6e8`, the
  directory spelling, is still not a valid gcloud argument.
- The default zone is `europe-west4-a` (`tpu.env`), moved from `us-east5-b` on 2026-08-10 because Compute
  Engine v6e quota is regional and this project holds none in us-east5 under the family metric. us-west4,
  where the v5e rig this was forked from provisions, has no v6e hardware, quota, or SKU at all.

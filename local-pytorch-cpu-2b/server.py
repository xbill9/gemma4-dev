...
reference checkpoint at float16. See docs/turing-aarch64-gap.md and tpu.env.
...
SERVICE_NAME = os.getenv("SERVICE_NAME", "jax-g4dn")
...
# g4dn topology: (GPUs, host RAM GiB). Read from `describe_instance_types`
# on 2026-08-28, not from a product page. Every size below 12xlarge carries a
# single T4; note 16xlarge is one GPU and 12xlarge is four, which is not the
# monotonic ladder the size suffix suggests.
_G4DN_SIZES = {
    "g4dn.xlarge": (1, 16),
    "g4dn.2xlarge": (1, 32),
    "g4dn.4xlarge": (1, 64),
...
def _user_data(model: str, instance_type: str) -> str:
  ...
  # empty /opt/jax-g4dn and no install log at all. Latent until 2026-08-26, when
  # making the swap threshold inclusive rendered this block for g4dn.xlarge --
...
async def list_g4dn_instances() -> str:
    """List instances tagged ManagedBy=gpu-jax-g4dn-2b."""
    try:
...
`g4dn.xlarge` in the two runs recorded under `benchmarks/runs/`; an unrecorded

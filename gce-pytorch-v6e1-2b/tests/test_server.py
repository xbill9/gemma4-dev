"""Smoke and regression tests for the gce-pytorch-v6e1-2b MCP server and repo invariants.
...
class ToolCatalogTests(unittest.TestCase):
    ...
    def test_action_and_type_enums_in_schema(self):
        ...
        for name in ("create_tpu_vm_instance", "find_tpu_vm"):
            workload = self.tools[name].input_schema["properties"]["workload"]
...
class StartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/startup_script_template.sh"
        ).read_text()
...
class PyTorchStartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/startup_script_pytorch_template.sh"
        ).read_text()
        return template.format(
            project_id="test-project",
            zone="europe-west4-a",
            ...
            hf_secret_id="hf-token",
            torch_pip_spec="tpu-backend-pkg",
            torch_index="us-python.pkg.dev/test-registry/simple/",
            torch_find_links="https://storage.googleapis.com/libtpu-releases/index.html",
            wheels_gcs="gs://test-project-wheels",
            torch_pip_extras="transformers xprof tensorboard tensorboard-plugin-profile",
            env_exports="FOO=bar BAZ=qux",
        )
    ...
    def test_renders_without_leftover_placeholders(self):
        ...
        self.assertIn("tpu-backend-pkg", rendered)
        self.assertIn("us-python.pkg.dev/test-registry/simple/", rendered)
        self.assertIn("TPU environment ready.", rendered)
        self.assertIn("FOO=bar", rendered)

    def test_smoke_test_covers_both_torch_stacks(self):
        """The embedded smoke test has to work on whichever stack the script installed.

        The two are mutually exclusive — torch_xla registers the "xla" device and the
        private backend registers "tpu" — so the smoke test branches on TORCH_DEVICE
        rather than picking one. The private branch must still never import the backend
        package directly; installing it is what registers "tpu".
        """
        rendered = self.render()
        ...
        self.assertEqual(
            imports, {"os", "torch", "torch_xla"}, f"unexpected imports in smoke test: {imports}"
        )
        # torch_xla is imported inside the "xla" branch only, and the private-backend
        # branch still compiles through backend="tpu".
        self.assertIn('DEVICE == "xla"', smoke)
        self.assertIn('backend="tpu"', smoke)
        self.assertIn("bfloat16", smoke)

    def test_public_path_needs_no_credentials(self):
        """With torch_index empty the script must take the public find-links branch and
        never reach for an Artifact Registry access token to install torch."""
        template = (
            ROOT / ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/startup_script_pytorch_template.sh"
        ).read_text()
        rendered = template.format(
            ...
            model_name="google/gemma-4-E2B-it",
            hf_secret_id="hf-token",
            torch_pip_spec="torch 'torch_xla[tpu]'",
            torch_index="",
            torch_find_links="https://storage.googleapis.com/libtpu-releases/index.html",
            wheels_gcs="",
            torch_pip_extras="xprof tensorboard tensorboard-plugin-profile",
            env_exports="",
        )
        self.assertIn('TORCH_INDEX=""', rendered)
        self.assertIn("libtpu-releases/index.html", rendered)
        self.assertIn('TORCH_DEVICE="xla"', rendered)

    def test_profiling_kit_is_complete(self):
        """xprof writes the trace, tensorboard views it, and the plugin is what teaches
        the viewer to render it. Any one alone leaves traces nothing can open."""
        rendered = self.render()
        for pkg in ("xprof", "tensorboard", "tensorboard-plugin-profile"):
            self.assertIn(pkg, rendered)
        self.assertIn("/opt/profiles", rendered)
...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_skill_snapshots_in_sync_with_sources(self):
        ...
        for src, snap in (
            ("server.py", ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/server.py"),
            ("project-setup.sh", ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/project-setup.sh"),
            ("requirements.txt", ".claude/skills/gce-pytorch-v6e1-2b-management/mcp/requirements.txt"),
        ):
            self.assertTrue(
    ...
    def test_plugin_copy_matches_skill(self):
        for rel in (
            ...
            self.assertTrue(
                filecmp.cmp(
                    ROOT / ".claude/skills/gce-pytorch-v6e1-2b-management" / rel,
                    ROOT / "skills/gce-pytorch-v6e1-2b-management" / rel,
                    shallow=False,
                ),
                f"skills/gce-pytorch-v6e1-2b-management/{rel} is stale — run `make skill`",
            )

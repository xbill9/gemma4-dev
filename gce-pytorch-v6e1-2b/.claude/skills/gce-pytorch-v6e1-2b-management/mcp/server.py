...
# Packages the pytorch-workload startup script installs. TPU_BACKEND_INDEX is the
# switch between the two torch stacks, and the startup script branches on it:
#
#   unset (the default) -> public PyTorch/XLA. `torch torch_xla[tpu]` from PyPI
#       plus the libtpu find-links index below; no credentials needed, and the
#       device registered is "xla".
#   set                 -> the private, operator-supplied backend. Its packages
#       are not publicly distributable, so the package name, the pip index host
#       and the wheel-mirror bucket are supplied at deploy time and never
#       committed here. The device registered is "tpu".
#
# On the private path, pin the CPU torch build in TPU_BACKEND_PIP_SPEC (e.g.
# "<pkg> torch==2.11.0+cpu"): the nightly backend wheels leave their torch
# dependency unconstrained, so pip otherwise resolves a newer torch whose ABI the
# extension .so was not built against (verified failure on 2.13). Never add plain
# `torch` on its own there — the backend index pulls the matching CPU build
# automatically. On the public path `torch` IS required, and is unpinned so the
# newest release wins.
TPU_BACKEND_INDEX = os.getenv("TPU_BACKEND_INDEX", "")
TPU_BACKEND_PIP_SPEC = os.getenv(
    "TPU_BACKEND_PIP_SPEC", "" if TPU_BACKEND_INDEX else "torch 'torch_xla[tpu]'"
)
# Where the public path resolves libtpu from. Only consulted when
# TPU_BACKEND_INDEX is unset.
TPU_BACKEND_FIND_LINKS = os.getenv(
    "TPU_BACKEND_FIND_LINKS", "https://storage.googleapis.com/libtpu-releases/index.html"
)
...
# Public-PyPI extras installed after the TPU stack: the model/tokenizer stack,
# the profiling kit, and the QAT / custom-kernel pieces (compressed-tensors
# checkpoints, Pallas via jax, chat templates need jinja2>=3.1).
# Best-effort at boot; failure is non-fatal.
#
# The profiling kit is all three of xprof, tensorboard and
# tensorboard-plugin-profile: xprof is the trace collector torch_xla's
# `xp.start_trace` writes through, tensorboard is the viewer, and the plugin is
# what teaches the viewer to render an xprof trace. Installing xprof alone gives
# you traces nothing can open.
TPU_BACKEND_PIP_EXTRAS = os.getenv(
    "TPU_BACKEND_PIP_EXTRAS",
    # setuptools<81: tensorboard/xprof need pkg_resources (removed in 81).
    "transformers accelerate huggingface_hub compressed-tensors 'jinja2>=3.1' jax "
    "'setuptools<81' xprof tensorboard tensorboard-plugin-profile",
)
...
def _get_pytorch_startup_script(zone: str, model_name: Optional[str] = None) -> str:
    """Formats the PyTorch startup script template: installs a torch TPU stack into a
    dedicated python3.12 interpreter (no venv) on the bare VM, installs the xprof +
    tensorboard profiling kit, stages the checkpoint, and runs a compile smoke test —
    no docker and no vLLM.

    Which stack it installs is decided by TPU_BACKEND_INDEX: unset means public
    PyTorch/XLA from PyPI (the default, no credentials), set means the private
    operator-supplied backend from its authenticated Artifact Registry index.

    Raises RuntimeError if the template is missing or malformed, or if the private
    path was selected without a package list; callers must not create
    infrastructure with a broken startup script.
    """
    if TPU_BACKEND_INDEX and not TPU_BACKEND_PIP_SPEC:
        raise RuntimeError(
            "TPU_BACKEND_INDEX is set but TPU_BACKEND_PIP_SPEC is empty, so the private "
            "backend path has no package list. Set both to use the private backend, or "
            "unset TPU_BACKEND_INDEX to take the public PyTorch/XLA path."
        )
    ...
    try:
        ...
        return template.format(
            ...
            torch_find_links=TPU_BACKEND_FIND_LINKS,
...
async def create_tpu_vm_instance(
    ...
    if workload == "pytorch":
        try:
            startup_script_content = _get_pytorch_startup_script(zone, selected_model)
        except RuntimeError as e:
            return f"❌ Aborted: {e}"
    else:
        min_chips = _min_chips_for_model(selected_model)
    ...
    if workload == "pytorch":
        return (
            ...
            f"installing `{TPU_BACKEND_PIP_SPEC}` "
            f"({'private backend' if TPU_BACKEND_INDEX else 'public PyTorch/XLA'}) plus the xprof/tensorboard "
            f"profiling kit, and staging `{selected_model}`. Follow with `wait_for_pytorch_ready` "

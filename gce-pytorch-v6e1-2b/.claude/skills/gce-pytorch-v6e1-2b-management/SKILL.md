...
target project and writes the `gce-pytorch-v6e1-2b` entry into the project's `.mcp.json`,
...
approve the server when prompted; `/mcp` should list `gce-pytorch-v6e1-2b`.

Manual alternative:

```bash
claude mcp add gce-pytorch-v6e1-2b \
  ...
  -- python .claude/skills/gce-pytorch-v6e1-2b-management/mcp/server.py
```

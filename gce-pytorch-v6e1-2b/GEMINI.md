# 🤖 Gemini Workspace Context: TPU Management Skill & gce-pytorch-v6e1-2b MCP Agent
...
This repository packages a Claude Code skill (`gce-pytorch-v6e1-2b-management`) and a **Model Context Protocol (MCP) server** (`gce-pytorch-v6e1-2b`) that together act as an AI DevOps/SRE agent for Google Cloud TPUs. Two main purposes:

1. **Infrastructure Operations:** Finding, provisioning, and destroying TPU capacity and running Gemma 4 on TPU VMs. This rig provisions from **Compute Engine** (`gcloud compute instances create --machine-type=ct6e-standard-1t --provisioning-model=FLEX_START`), not the Cloud TPU API, and its default workload is **PyTorch/XLA** (`torch` + `torch_xla[tpu]`, device `xla`) rather than vLLM.
...
> **Note.** `CLAUDE.md` is the authoritative agent-instruction file where these three disagree. The LiteLLM proxy configuration below assumes an OpenAI-compatible endpoint on `:8000`, which only the `workload="vllm"` path serves — the default PyTorch/XLA path runs benchmarks and profiling directly on the VM and exposes no HTTP API.
...
- **Reference guide:** `.claude/skills/gce-pytorch-v6e1-2b-management/references/tpu-guide.md` — TPU getting started guide: zones, quotas, troubleshooting
...
make skill-package # ...and rebuild dist/gce-pytorch-v6e1-2b-management-skill.zip
```

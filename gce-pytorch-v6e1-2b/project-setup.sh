#!/usr/bin/env bash
# One-command setup of the gce-pytorch-v6e1-2b-management skill + this rig's MCP server for a project.
#
# Usage:
#   ./project-setup.sh [TARGET_DIR] [options]
#
#   TARGET_DIR              Project to set up (default: current directory).
#                           Installs the skill into TARGET_DIR/.claude/skills/gce-pytorch-v6e1-2b-management
...
#   - the skill repo root (uses .claude/skills/gce-pytorch-v6e1-2b-management), or
...
if [ -f "$SCRIPT_DIR/.claude/skills/gce-pytorch-v6e1-2b-management/SKILL.md" ]; then
  SKILL_SRC="$SCRIPT_DIR/.claude/skills/gce-pytorch-v6e1-2b-management"     # repo root
  ...
  err "cannot find the gce-pytorch-v6e1-2b-management skill next to $SCRIPT_DIR"
fi
...
if [ "$GLOBAL" -eq 1 ]; then
  SKILL_DEST="$HOME/.claude/skills/gce-pytorch-v6e1-2b-management"
else
  SKILL_DEST="$TARGET_DIR/.claude/skills/gce-pytorch-v6e1-2b-management"
fi
...
config.setdefault("mcpServers", {})[name] = {
    "command": python_bin,
    "args": [".claude/skills/gce-pytorch-v6e1-2b-management/mcp/server.py"],

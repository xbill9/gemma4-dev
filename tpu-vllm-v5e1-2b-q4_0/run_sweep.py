...
# The template rig hardcoded its own server's public IP here, which would have pointed this
# rig's sweep at the bf16 deployment. Set VLLM_HOST to this rig's endpoint before running.
...
for c in concurrencies:
    for ctx in contexts:
        ...
        try:
            ...
            cmd = f"vllm bench serve --host {VLLM_HOST} --port 8000 --model google/gemma-4-E2B-it-qat-q4_0-unquantized --dataset-name random --num-prompts {num_prompts} --random-input-len {ctx} --random-output-len 100 --max-concurrency {c}"

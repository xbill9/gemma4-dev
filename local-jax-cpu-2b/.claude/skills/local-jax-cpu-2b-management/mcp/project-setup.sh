#!/usr/bin/env bash
# Install the bundled skill and register this rig's local-CPU MCP server.
set -euo pipefail
...
JAX_HOST="${JAX_HOST:-127.0.0.1}"
JAX_PORT="${JAX_PORT:-8000}"
SERVER_NAME=""
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --host) JAX_HOST="$2"; shift ;;
    --port) JAX_PORT="$2"; shift ;;
    ...
    -h|--help)
      ...
      echo "                           This server has NO authentication; binding"
      echo "                           0.0.0.0 puts inference on the network."
      echo "  --port PORT              Serving port (default: 8000)"
      echo "  --server-name NAME       MCP server name; the key it is registered under, which"
      echo "                           prefixes every tool as mcp__<name>__start_jax_server."
      echo "                           (default: the rig directory, local-jax-cpu-2b)"
      echo "  --skip-deps              Skip the dependency import check"
      echo ""
      echo "There are no cloud options. This rig provisions nothing: no region,"
      echo "no profile, no instance type. The hardware is the machine you are on."
      exit 0 ;;
...
case "$JAX_PORT" in
  ''|*[!0-9]*) err "--port must be a number: $JAX_PORT" ;;
esac
...
# No boto3 here, unlike every cloud sibling. The control plane needs mcp and
# httpx; the SERVING stack (jax, fastapi, transformers, ...) is checked by the
# check_dependencies tool against the interpreter that will actually run it,
# which is the failure this rig has that the cloud ones do not.
if [ "$SKIP_DEPS" -eq 0 ] && ! python3 -c 'import httpx,mcp' >/dev/null 2>&1; then
  echo "warning: install dependencies with: python3 -m pip install -r $SKILL_DEST/mcp/requirements.txt" >&2
fi

ENV_JSON="$(python3 - "$MODEL_NAME" "$JAX_HOST" "$JAX_PORT" "$SERVER_NAME" <<'PY'
import json, sys
model, host, port, server_name = sys.argv[1:]
...
print(json.dumps({"MODEL_NAME": model, "JAX_HOST": host, "JAX_PORT": port,
...
info "registered MCP server '$SERVER_NAME' — serving $MODEL_NAME on $JAX_HOST:$JAX_PORT"
echo "Restart Claude Code, then run check_host_capacity before starting a serve."

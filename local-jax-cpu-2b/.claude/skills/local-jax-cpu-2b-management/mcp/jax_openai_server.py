...
def metrics():
    ...
    lines = [
        *precision,
        ...
        "",
        # Host RSS is exported UNCONDITIONALLY, on every rig. On an accelerator
        # it is a secondary reading; on a device with no allocator it is the only
        # one there is, and it is the number that decides whether the process
        # fits. Same series name everywhere so a dashboard does not need to know
        # which kind of rig it is pointed at.
        "# HELP tpu_jax_host_rss_bytes Resident set size of the serving process",
        "# TYPE tpu_jax_host_rss_bytes gauge",
        f'tpu_jax_host_rss_bytes{{model="{MODEL_ID}"}} {mem.get("host_rss_bytes", 0)}',
        "",
    ]
    # The HBM pair is emitted ONLY when a device allocator actually answered.
    # Exporting `tpu_jax_hbm_used_bytes 0` on a CPU device would not be a small
    # inaccuracy: on an accelerator that value means "the allocator holds
    # nothing", which is a symptom, and here it would mean "there is no
    # allocator", which is not. An absent series is the honest rendering of a
    # question the platform cannot answer, and it is also what keeps
    # `tpu_jax_host_rss_bytes` from looking like a redundant duplicate.
    if mem.get("has_device_allocator"):
        lines += [

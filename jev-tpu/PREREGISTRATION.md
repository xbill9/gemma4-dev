...
- **2026-09-24, after one E2B smoke read and before any scored read (deviation 2):** vLLM on TPU returns only the top-k log-probabilities; `tpu_inference` has no per-request `logprob_token_ids`, and a request carrying it came back HTTP 500 `list index out of range`. The read therefore asks for the top 32 log-probabilities and takes the label tokens from them. A label inside the 32 gets its exact log-probability, the same number the explicit request returned on the L4; a label outside gets the proxy's floor (the lowest returned value minus 5), and every record keeps `labels_returned`. The share of reads with every label returned is reported per arm. The smoke check becomes "at least one label returned in every smoke record". Four raw request shapes are sent and kept verbatim before each arm's reads.

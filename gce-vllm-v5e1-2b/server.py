"""MCP devops agent for serving Gemma 4 E2B with vLLM on one TPU v5e chip, provisioned
through **Compute Engine** rather than the Cloud TPU API.

READ THIS BEFORE PROVISIONING. Whether v5e can be provisioned this way is an **open
question, and this rig is the apparatus for settling it** — not a working deployment path
with a track record. Nothing here has ever created an instance. The evidence, in full:

  * Google's own v5e page says "TPU v5e is supported using Google Kubernetes Engine and the
    Cloud TPU API" — Compute Engine is not named. The Compute Engine machine-types page
    enumerates TPU7x, v6e and v5p, and `ct5lp` appears nowhere on it.
  * That same v5e page nonetheless documents `ct5lp-hightpu-1t` (24 vCPU / 48 GB) as a VM
    type, `ct5lp-hightpu-{1,4,8}t` exist as real machine types in 26 zones, the shared OS
    image family is literally named `…-tpu-v5e-v5p-v6e`, and there is a v5-lite quota on
    `compute.googleapis.com`.
  * Every one of those positives is explainable without a CE path: the Cloud TPU API and
    GKE are both implemented *on* Compute Engine, so its TPU VMs are GCE instances booting
    GCE images against GCE quota whether or not `instances create` accepts the type
    directly. GKE in particular consumes `ct5lp-*` by name in node pools.

So the working answer stays **no**, and it is a documented-but-unattempted no. One
`create_tpu_instance` settles it: rejection at validation is free and conclusive, acceptance
bills until deleted. Record the result in ../HARDWARE.md and ../NAMING.md when someone runs
it — it decides whether six v5e rigs have a migration path at all.

Everything below is written as if the answer is yes, because that is the only way to run
the experiment.
"""
...
# The Compute Engine machine type is the real accelerator request. `ct5lp-hightpu-1t` is one
# v5e chip, 24 vCPU / 48 GB — a third of the v6e shape's host, which is worth knowing before
# assuming a v6e-tuned client-side benchmark harness fits on it.
#
# **Unlike v6e and v5p, v5e publishes no `<name>-tpu` twin.** Do not read that as evidence
# either way about the CE path: ../HARDWARE.md records that the *bare* shapes are the
# documented creatable ones, so a missing `-tpu` variant proves nothing. What it does mean is
# that there is exactly one string to choose here.
MACHINE_TYPE = os.getenv("MACHINE_TYPE", "ct5lp-hightpu-1t")
# Replaces the TPU API's --runtime-version (`v2-alpha-tpuv5-lite` on the sibling rig). Ubuntu
# 22.04 / kernel 6.8, shared across v5e/v5p/v6e — note the family name says v5e out loud, one
# of the few positive signals that this path covers the chip at all. Pin the *family*, never a
...
# different pools.** The TPU API ids below are what the sibling tpu-vllm-v5e1-2b uses; they
...
# What Compute Engine actually meters. **The v6e rig's asymmetry does not hold here**, and
# this is the one place the v5e retarget is not a search-and-replace: v6e has only a
# preemptible per-zone id and has to fall back to the regional family-wide quota for
# everything else, whereas v5 Lite PodSlice publishes *both* halves per-zone. So this rig
# points at the per-zone non-preemptible id, which is the dimension find_tpu's sweep wants.
#
# Verified against Cloud Quotas 2026-08-10: both ids exist on compute.googleapis.com, each
# with a single default bucket reading -1 across 130 locations and no per-zone override on
# this project. `TPUS-PER-TPU-FAMILY-per-project-region` has **no CT5LP bucket at all** here
# (it holds CT6E=32 in eight regions, CT3P and CT3) — which is why it is not the default.
GCE_QUOTA_ID = os.getenv("GCE_QUOTA_ID", "TPU-LITE-PODSLICE-V5-per-project-zone")
GCE_SPOT_QUOTA_ID = os.getenv("GCE_SPOT_QUOTA_ID", "PREEMPTIBLE-TPU-LITE-PODSLICE-V5-per-project-zone")
# Family name for the regional TPUS-PER-TPU-FAMILY quota, reported by the tools for
# comparison. v5e is CT5LP there — the machine-type prefix, not the `v5litepod` the TPU API
# uses and not the `v5e` of prose.
GCE_TPU_FAMILY = os.getenv("GCE_TPU_FAMILY", "CT5LP")
...
# prefix there, and calls spot "Preemptible". For v5e in us-west4 that reads `TpuV5e running
# in Las Vegas`, `DWS Defined Duration V5e running in Las Vegas`, and `TpuV5e attached to Spot
# Preemptible VMs running in Las Vegas`, so tpu_type is the only thing a chip retarget changes
# here. Verified against the live catalog 2026-08-06.
#
# **The catalog is billing, not the control plane.** These are the same SKUs the Cloud TPU API
# bills against — a v5e price existing says nothing about whether `compute instances create`
# will accept a `ct5lp` machine type. See the header note on this rig's open question.
...
def _provisioning_flags(
    ...
    # Every instance carries rig=<RIG_NAME>. Sibling rigs provision TPU instances into this
    # same project, and unlike a Queued Resource id an instance name is not forced
...
async def _list_queued_resources_json(zone: str) -> Optional[list]:
    ...
    Retained on the Compute Engine rig **only to detect cross-path collisions**, and that
    matters more here than on the v6e twin: `us-west4-a` is the one zone this project has
    ever provisioned v5e in, and tpu-vllm-v5e1-2b, tpu-jax-v5e1-2b, tpu-pytorch-v5e1-2b and
    tpu-pytorch-v5e1-12b all queue resources into it. A chip any of them holds is a chip this
    rig cannot get. Nothing here creates or deletes one.
    """
    cmd = [
        "gcloud",
        "alpha",
...
# PATH at first boot, unlike the Cloud TPU API's runtime versions (v2-alpha-tpuv5-lite here,
# v2-alpha-tpuv6e there), which do. Verified 2026-08-10 on the first instance the v6e twin
# ever created — same image family, so it applies unchanged: the boot-time startup
...
def _ssh_command(instance_name: str, remote_cmd: str, zone: str = ZONE, ensure_docker: bool = False) -> list[str]:
    ...
    **This is `compute ssh`, not `compute tpus tpu-vm ssh`.** A ct5lp instance is an ordinary
    ...
    if rc != 0:
        ...
        if "TPUS_PER_TPU_FAMILY" in err or "tpus_per_tpu_family" in err:
            hint = (
                ...
                f"{REGION} — a *different pool* from the TPU API quota the sibling rig uses, and this "
                f"project has no {GCE_TPU_FAMILY} bucket in it at all. Request it in the Compute Engine "
                "quota page, not the TPU API one.)"
            )
...
async def _list_tpu_vm_nodes(zone: str = ZONE) -> list[dict]:
    ...
    hand-provisioned tpu-vm both land in that namespace. Instances created with a `ct5lp-*`
...
async def _active_qr_node_ids(zone: str = ZONE) -> set[str]:
    ...
    Nothing this rig creates appears here — a `ct5lp-*` instance has no Queued Resource
...
async def manage_tpu_instance(
    ...
    **Deliberately narrower than the sibling rig's `manage_queued_resource`, which deletes
    ...
    rigs now provision TPU instances into this project. So this only deletes instances
...
async def create_tpu_instance(
    ...
    provisioning_model is one of:
      ...
      * 'spot' — `SPOT`, reclaimable with ~30s notice, metered by the separate
        `PREEMPTIBLE-TPU-LITE-PODSLICE-V5-per-project-zone` Compute Engine quota. On v5e spot
        has historically undercut flex-start, which is the *reverse* of the v6e ordering in
        us-east5 — read `estimate_deployment_cost` rather than assuming either way.
...
async def get_zones_with_available_quota(
    ...
    **The service defaults to `compute.googleapis.com`, not `tpu.googleapis.com`** — that is
    ...
    holding one buys you nothing on the other.

    **v5e's Compute Engine ids are symmetrical where v6e's are not** — both
    `TPU-LITE-PODSLICE-V5-per-project-zone` and its `PREEMPTIBLE-` twin are per-zone, so
    unlike the v6e rig this one never has to fall back to the regional family-wide quota.
    Verified 2026-08-10: both exist, both report a single default bucket of -1 over 130
    locations with no per-zone override on this project, and
    `TPUS-PER-TPU-FAMILY-per-project-region` holds no CT5LP bucket at all.

    A -1 default is not a grant. It means nobody has ever set this quota on this project,
    which reads identically here whether the real ceiling is unlimited or zero.
...
async def _zones_with_machine_type(machine_type: str = MACHINE_TYPE) -> list[str]:
    ...
    the right gate-1 signal for this path. `ct5lp-hightpu-1t` is published in **26 zones**
    (verified 2026-08-10), `us-west4-a` among them. For v6e the two catalogs agree exactly
    (18 zones each); for v5p they disagree in one zone. See ../HARDWARE.md.

    **For v5e, gate 1 passing is exactly the ambiguity this rig exists to resolve.** The
    machine type being published is one of the four signals ../HARDWARE.md weighs, and all
    the positive ones are explainable by the Cloud TPU API being implemented on Compute
    Engine underneath. A zone appearing here is not a promise that `instances create` will
    take the type.
    """
    cmd = [
        "gcloud",
...
async def find_tpu(
    ...
    **The zone source differs from the sibling rig.** That rig sweeps zones with non-zero TPU
    API quota. Here the sweep is driven by where the machine type is published instead, which
    is the stronger gate-1 signal: v5e's Compute Engine quota ids are per-zone but carry only
    an unset -1 default on this project, so filtering on them would admit every zone and
    prove nothing. (On the v6e twin the quota is regional and cannot produce a zone list at
    all — different reason, same conclusion.)
...
async def list_queued_resources(zone: str = ZONE) -> str:
    ...
    Nothing this rig creates appears here. It is kept because four v5e rigs share this
    project and `us-west4-a` and compete for the same physical chips, so "who is holding the
    v5e" is a question this rig has to be able to answer about the other control plane.
    """
    cmd = [
        "gcloud",
        "alpha",

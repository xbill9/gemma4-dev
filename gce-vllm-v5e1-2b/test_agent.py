...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_vllm_deployment_config(self, mock_run_command, mock_get_secret):
        ...
        self.assertNotIn("--version=v2-alpha-tpuv5-lite", config)
...
class TestProvisioningModel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_uses_screaming_case_provisioning_model(self, mock_run_command, mock_get_secret):
        ...
        ok, msg = await _create_tpu_instance("inst-test", "us-west4-a", "spot")
        ...
        self.assertIn("--machine-type=ct5lp-hightpu-1t", cmd)
        self.assertNotIn("--accelerator-type=v5litepod-1", cmd)
        self.assertNotIn("--runtime-version=v2-alpha-tpuv5-lite", cmd)
    ...
    async def test_spot_and_on_demand_also_get_a_run_bound(self, mock_run_command, mock_get_secret):
        ...
        for model in ("spot", "on-demand"):
            await _create_tpu_instance("inst-test", "us-west4-a", model)
            cmd = mock_run_command.call_args[0][0]
    ...
    async def test_create_defaults_to_flex_start(self, mock_run_command, mock_get_secret):
        ...
        ok, _ = await _create_tpu_instance("inst-test", "us-west4-a")
    ...
    async def test_create_labels_the_instance_with_its_rig(self, mock_run_command, mock_get_secret):
        ...
        await _create_tpu_instance("inst-test", "us-west4-a")
    ...
    async def test_create_rejects_unknown_model_before_calling_gcloud(self, mock_run_command, mock_get_secret):
        ok, msg = await _create_tpu_instance("inst-test", "us-west4-a", "preemptible")
    ...
    async def test_create_timeout_is_not_reported_as_a_failure(self, mock_run_command, mock_get_secret):
        ...
        ok, msg = await _create_tpu_instance("inst-test", "us-west4-a")
...
# Shapes and rates copied from the live catalog for us-west4 on 2026-08-06 — the same
# fixtures the TPU-API v5e rig asserts against, because **the catalog does not fork with the
# control plane**. Both rigs bill the identical SKUs; only the API that provisions differs.
# The decoys are the real reason this matcher needs testing: the Reserved and Commitment
# SKUs describe the same chip in the same region and two share the OnDemand usage type, so
# only the ^ anchor keeps them out.
FAKE_SKUS = [
...
class TestCostFromLivePricing(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ranking_matches_published_rates(self):
        """On v5e in us-west4 spot is the cheapest and flex-start lands just above it.

        That is the reverse of the v6e ordering on the rig this one was forked from, which
        is the point: the ranking is a fact about the catalog for one chip in one region,
        never a rule about the provisioning models. Assert what is published, not what is
        assumed.
        """
        rates = {}
    ...
    async def test_reservation_bound_reports_no_rate_rather_than_guessing(self):
        ...
        rate, _, note = await _lookup_tpu_rate("v5e", "reservation-bound", "us-west4")
        self.assertIsNone(rate)
    ...
    def test_topology_parsing(self):
        ...
        self.assertIsNone(_parse_topology("v5litepod-1"))
...
def _node(name, ip="10.0.0.1", status="RUNNING", external=True, rig=None):
    ...
    return {
        "name": f"projects/p/zones/us-west4-a/instances/{name}",
        "status": status,
        "machineType": "projects/p/zones/us-west4-a/machineTypes/ct5lp-hightpu-1t",
...
class TestNodeDiscovery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_ct5lp_instance_is_not_a_tpu_vm_node(self):
        ...
        async def _run(cmd, timeout=60):
            ...
            if "queued-resources" in cmd:
                return 0, "[]", ""
            if "instances" in cmd and "list" in cmd:
                return 0, json.dumps([_node("gce-vllm-v5e1-2b")]), ""
            return 1, "", "unexpected"
    ...
    async def test_no_remote_tool_reaches_the_vm_through_tpu_vm_ssh(self):
        ...
        That cannot reach a ct5lp instance — it is not a TPU VM node — so all four failed with
        a not-found against a VM that was plainly RUNNING. The old
        `test_a_ct5lp_instance_is_not_a_tpu_vm_node` did not catch it because it only
        ...
        with (
            patch("server.run_command", new=AsyncMock(side_effect=_run)),
            patch("server._resolve_node_id", new=AsyncMock(return_value="gce-vllm-v5e1-2b")),
        ):
            await server.manage_vllm_docker("gce-vllm-v5e1-2b", action="status")
            await server.get_vllm_docker_logs("gce-vllm-v5e1-2b")
            await server.get_tpu_system_logs("gce-vllm-v5e1-2b")
    ...
    async def test_docker_commands_install_docker_when_the_image_lacks_it(self):
        ...
        with (
            ...
            await server.manage_vllm_docker("gce-vllm-v5e1-2b", action="start")
            docker_cmd = seen[-1][-1]
            seen.clear()
            await server.get_tpu_system_logs("gce-vllm-v5e1-2b")

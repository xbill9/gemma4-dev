# CLAUDE.md — TPU Management Skill & tpu-jax-v5e1-2b MCP Agent
...
- **Hand-maintained (not regenerated):** `.claude/skills/tpu-jax-v5e1-2b-management/SKILL.md`, `.claude/skills/tpu-jax-v5e1-2b-management/mcp/startup_script_template.sh`, `.claude/skills/tpu-jax-v5e1-2b-management/mcp/startup_script_jax_template.sh`
- **Generated by `make skill` (`refresh_skill.py`):**
  - `.claude/skills/tpu-jax-v5e1-2b-management/mcp/{server.py,project-setup.sh,requirements.txt}` — copies of the root sources
  - `.claude/skills/tpu-jax-v5e1-2b-management/references/tpu-guide.md` — `tpu.md` with base64 screenshots stripped
  - `skills/tpu-jax-v5e1-2b-management/` — full copy of the skill in plugin layout (for the marketplace)
...
- **Plugin marketplace:** `.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json` make this rig installable. It now ships from the monorepo — `/plugin marketplace add xbill9/gemma4-dev`, then install the `tpu-jax-v5e1-2b` plugin. The marketplace that `/plugin` actually reads is the one at the **monorepo root** (`../.claude-plugin/marketplace.json`), which lists all three rigs; the copy in this directory only matters if this rig is published standalone again. Keep the two in sync. Validate with `claude plugin validate .`
- **Distribution zip:** `dist/tpu-jax-v5e1-2b-management-skill.zip`, built by `make skill-package`; committed so the raw-GitHub-URL install works — rebuild it when the skill changes
...
make skill-package # Refresh + rebuild dist/tpu-jax-v5e1-2b-management-skill.zip
...
The server's config comes from env vars: `GOOGLE_CLOUD_PROJECT`, `MODEL_NAME`, `ACCELERATOR_TYPE`, `TENSOR_PARALLEL_SIZE`, `TPU_RUNTIME_VERSION` (overrides the per-family queued-resource image), `JAX_PYTHON_VERSION` / `JAX_PIP_SPEC` / `JAX_PIP_EXTRAS` (interpreter and packages for `workload="jax"` VMs).

**Default deployment target: a single-chip v5e flex-start VM** — `ACCELERATOR_TYPE=v5e-1`
(`ct5lp-hightpu-1t`, 16 GB HBM), `TENSOR_PARALLEL_SIZE=1`, `GOOGLE_CLOUD_ZONE=us-west4-a`,
`MODEL_NAME=google/gemma-4-E2B-it`. See `deploy.md` for the provisioning commands and the
HBM budget that decides which QAT checkpoint fits. All *measured* numbers in this repo
(README, `benchmarks/runs/**`) were taken on a v6e-1 and are not v5e-1 results.

## Technical Standards: Serving Flags

**Default path — pure JAX on the v5e-1.** `jax_openai_server.py`, no docker, no vLLM:
...
v5e-1, 8 on a v6e-8), then apply these parameters for stable Gemma 4 tool integration:
...
- `.claude/skills/tpu-jax-v5e1-2b-management/SKILL.md` — the skill: standard lifecycle, MCP tool catalog, required vLLM flags, field notes from live deployments, cautions (read this before touching TPU provisioning logic)

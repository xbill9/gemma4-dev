...
# v6e (Trillium) keeps its marketing name in the TPU API — no "litepod" rename the way v5e
# has; the matching Flex-start runtime is v2-alpha-tpuv6e.
TPU_RUNTIME_VERSION = os.getenv("TPU_RUNTIME_VERSION", "v2-alpha-tpuv6e")
...
# about this id instead of TPU_QUOTA_ID. Both ids confirmed present on the TPU API 2026-08-07
# — note v6e drops the "Litepod" the v5e ids carry, so neither id survives a chip retarget.
...
# model this rig has provisioned in production. Which zones accept it for v6e-1 has not been
# established by attempting creation the way it was on v5e — the catalog publishes a
# flex-start V6e rate in us-east5 (see CLAUDE.md), which is a price, not an offer of
# capacity. The other two models exist for when a zone refuses.
...
async def create_tpu_queued_resource(
    ...
    provisioning_model is one of:
      * 'flex-start' (default) — queues for scarce capacity, self-terminates after 4h.
        Not every zone accepts it: the catalog publishes a flex-start V6e rate in 8
        regions and the v5e experience was that other zones reject it at the API.
...
async def get_zones_with_available_quota(
    ...
    Non-zero quota does NOT mean the zone will accept the request. 37 zones report v6e
    quota on this project, only 8 regions publish a flex-start V6e rate, and fewer still
    have v6e-1 hardware. Quota is a ceiling, not an offer of capacity.

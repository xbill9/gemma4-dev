...
(`tpu-2B-v6e1-devops-agent`, the pre-rename node) reported as "No ACTIVE Queued Resource found" and every
...
prefixes every tool: `mcp__tpu-vllm-v6e1-2b__find_tpu`. All six rigs used to register as `tpu-devops`, so a
...
tools manage a Queued Resource of the same name whose *node* is `tpu-vllm-v6e1-2b-node`. Those are two
...
VMs created before this change are named `tpu-2B-v6e1-devops-agent` and the pre-existing Queued Resource id
was `vllm-gemma4-qr`; reach them with `make status SERVICE_NAME=tpu-2B-v6e1-devops-agent` or by passing
...
**A `v5e` label in this rig is a fork artifact, not a claim about the hardware.** The rig was forked from
`tpu-vllm-v5e1-2b` and retargeted to v6e-1 on 2026-08-07; the config, tests, and docs were swept, but
everything that *records a measurement* was deliberately left alone, because a benchmark names the hardware
it ran on and relabelling one would falsify it. So `benchmarks/runs/*-v5e1/`, `benchmarks/reports/`,
`plot_sweep_v5e1.py` (which reads an archived v5e CSV), and `v5e.md` + `v5e-cover.jpg` (a published article
about the v5e rig) all still say v5e and are all correct. **Nothing in `benchmarks/` here was measured on
v6e-1.**

`compare_chips.py`, `compare_benchmarks.py`, and `plot_grid.py` carry v6e labels but are not evidence
either way: they were copied over unchanged from a v6e-era fork and hardcode "v6e-4"/"v6e-1" titles while
reading CSVs out of sibling `../tpu-*-v6e*-devops-agent/` directories that this rig does not own.
`benchmark_tables.md` is likewise a v6e-era report from that fork. Both sets of labels describe where the
file came from, not what this rig is.

**`us-west4` has no v6e at all**, which is why the retarget had to move the zone. Verified 2026-08-07:
no `v6e-1` in `gcloud compute tpus accelerator-types list` for any us-west4 zone, no us-west4 row in the
v6e quota, and no v6e SKU in the billing catalog for the region. The v5e rig's `us-west4-a` default — the
one zone that accepted flex-start `v5litepod-1` — is therefore useless here. The default is now
...
- **Spot is metered by a different quota**, `TPUV6EPreemptiblePerProjectPerZoneForTPUAPI`
...
Scheduler) and drops the `Tpu` prefix (`DWS Defined Duration V6e`), and spot is `usageType: Preemptible`
spelled `TpuV6e attached to Spot Preemptible VMs`. Both spellings survived the v5e→v6e move unchanged, so
`tpu_type` is the only thing the retarget touched here. The `Reserved …`, `Commitment v1: …`, and
**`Capacity Optimized TpuV6e …`** SKUs describe the same chip in the same region and three of them are also
`OnDemand`, so the patterns are anchored with `^`.

**Don't assume spot is the cheapest.** In us-east5 v6e flex-start lists at $1.35/chip-hr and spot at
$1.4033 — spot is *dearer*, the reverse of the v5e ordering this rig was forked from. `tpu_type` defaults
to `v6e`; the ranking is a fact about one chip in one region, so read it out of the tool rather than
assuming. A price existing does not mean capacity is obtainable.
...
and the Makefile uses `?=` — so `make status ZONE=...` still works for a one-off. Defaults are `us-east5-b` /
...
--topology=1x1` (the `--type`/`--topology` pair spells the same chip as `--accelerator-type=v6e-1`). All
three live in one place each — `ACCELERATOR_TYPE` / `TPU_RUNTIME_VERSION` in `server.py`, the Makefile flags
— and all are env-overridable. This makes the directory slot and the gcloud flag coincide at `v6e`, which
they did not on v5e. That coincidence is not a licence to copy slots into flags: `v6e1` (the slot spelling,
no hyphen) is still not a valid gcloud value, and the next chip may rename itself again. Read
`ACCELERATOR_TYPE` out of `tpu.env`.

The quota ids move too, and lose the `Litepod` the v5e ids carry:
`TPUV6EPerProjectPerZoneForTPUAPI` and `TPUV6EPreemptiblePerProjectPerZoneForTPUAPI`.
...
## Articles: two variants per piece, and the Medium one is generated

Each published piece exists twice — `devto-<slug>.md` and `devto-<slug>-medium.md` — because the two
platforms disagree about markdown. `devto-vllm-gemma4-e2b-v6e1.md` is the source; the `-medium.md` file
is **generated by `to_medium.py` and should not be hand-edited**, exactly like `GemmaTools.md`. Regenerate
with:

```
python3 to_medium.py devto-vllm-gemma4-e2b-v6e1.md devto-vllm-gemma4-e2b-v6e1-medium.md \
  --cover "<raw-github-url>/v6e-cover.jpg" \
  --figure "<raw-github-url>/docs/assets/v6e-asymmetry.png|caption" \
  --figure "<raw-github-url>/docs/assets/v6e-no-knee.png|caption"
```

**Never hard-wrap article prose.** House style across every article in the monorepo is **one paragraph
per line** — `devto-vllm-gemma4-e2b-v5e1.md` and its `-medium.md` both run to 783 characters on a line.
A file wrapped at 100 columns is the odd one out and reads as a diff-noise generator, since re-flowing one
sentence rewrites the whole paragraph. Code blocks, tables, headings and list items keep their own line.
This applies to the dev.to and Medium variants alike; the converter preserves whatever wrapping it is
given, so fix the source, not the output.

What the conversion actually does, and why each step is needed:

- **Pipe tables → space-aligned fenced code blocks.** Medium does not render markdown tables at all; it
  flattens them into run-on prose and the numbers become unreadable. It *does* render fenced blocks in
  monospace, so alignment survives. Numeric columns are right-aligned, text columns left.
- **Markup inside table cells is flattened** — `**bold**`, backticks, and link syntax are stripped, and
  `×`/`≥`/`≤`/`→` transliterated. Nothing inside a code block is interpreted, so leaving them in would
  print the asterisks literally. Prose outside tables keeps its markdown untouched.
- **Front matter is dropped.** Medium has no equivalent, and `series:` / `cover_image:` / `published:`
  are dev.to-only keys. The cover becomes a plain image at the top of the body instead.
- **HTML comments are dropped.** Author scaffolding (`<!-- fill this URL at publish time -->`) renders as
  visible body text on Medium.

**Figures are generated too** — `plot_v6e_article.py` reads
`benchmarks/runs/2026-08-10-article-validation-v6e1/results/` rather than restating numbers, so a re-run
updates the plots. Output lands in `docs/assets/`. Colour encodes regime, never rank.

**Both `to_medium.py` and `plot_v6e_article.py` are top-level modules, so they are silently unlinted**
until added to the hardcoded `ruff check` list in the Makefile — see the `make lint` note above.

**Article image URLs are raw-GitHub links into this repo**, so a cover or figure only resolves once it is
committed *and pushed to `main`*. Both articles carry an HTML comment marking the one link that cannot be
filled until the sibling piece is published; the comment survives in the dev.to file and is stripped from
the Medium one.
...
**Every one of them came in with the fork; none was measured on this rig's v6e-1 hardware.** The `-v5e1`
dirs were measured on the v5e rig this was forked from, and the `-v6e1` ones came from a v6e-era fork
before that — so the two `-v6e1` legacy grids are the right chip by accident, not a baseline for here
(different rig, different vLLM, undated). Until a sweep is run on this rig, it has no numbers of its own.

| Run dir | What |
|---|---|
| `2026-08-07-kv-quant-v5e1` | fp8 KV cache experiment on `tpu-2B-v5e1-devops-agent` in us-west4-a; `ROLLBACK.md` is the way back for the *v5e* rig, not this one |
| `2026-08-06-vllm-sweep-v5e1` | The v5e rig's first conformant run — 12 measured + 4 infeasible cells, schema 1.1 report in `benchmarks/reports/` |
...
`benchmarks/INDEX.md` still carries the `tpu-vllm-v5e1-2b` heading: it is generated by the monorepo
`benchmarks/rollup.py`, so it corrects itself on the next `make benchmarks-rollup` from the root, once this
rig is tracked. Don't hand-edit it.

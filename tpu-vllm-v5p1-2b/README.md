...
## Current Deployment
*   **Model:** `google/gemma-4-E2B-it` on TPU v5p, **1 chip** via Compute Engine (`ct5p-hightpu-1t-tpu`).
...
To deploy and run this project, you need to address two main components: the **Inference Stack** (vLLM on TPU v5p) and the **MCP Server** itself.
...
*   **Hardware:** Cloud TPU v5p, one chip (95 GiB HBM), machine type `ct5p-hightpu-1t-tpu`.
...
*   **Image:** family `ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e` from project `ubuntu-os-accelerator-images` (Ubuntu 22.04, kernel 6.8). Pin the family, never a dated build.
...
*   `GOOGLE_CLOUD_ZONE`: Zone to provision and discover in (defaults to `us-east5-a`, the only v5p zone
    that offers flex-start).
...
*   `MACHINE_TYPE`: Compute Engine TPU machine type (defaults to `ct5p-hightpu-1t-tpu` — one v5p chip; the
    `-tpu` suffix marks the CE-native shape and is required for the 1-chip size).
*   `IMAGE_FAMILY` / `IMAGE_PROJECT`: boot image (defaults to `ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e` in
    `ubuntu-os-accelerator-images`).
*   `TPU_QUOTA_ID`: Cloud Quotas id scanned by `find_tpu` (defaults to `TPU-V5P-per-project-zone` on
    `compute.googleapis.com` — the TPU API's quotas meter the other, deprecated control plane).
...
*   `MCP_SERVER_NAME`: Name this server advertises, and the key it must be registered under — it prefixes
    every tool as `mcp__<name>__find_tpu` (defaults to the rig directory name, `tpu-vllm-v5p1-2b`). Set it
...
-   **Optimization Flags:**
    -   `--tensor-parallel-size 1` (one chip, 95 GiB — nothing to shard)
    -   `--max-model-len 16384`
...
## Flex-start VMs
Our stack leverages **Flex-start VMs** (via Compute Engine flex-start VMs) to maximize TPU availability and minimize costs.

### Key Characteristics
*   **Dynamic Workload Scheduler (DWS):** Provisions resources from a secure pool, increasing the probability of securing high-demand TPU v5p chips.
...
*   **`find_tpu`**: Scans zones for available v5p quota and provisions the Queued Resource in the first one that takes it.
    Only `us-central1-a`, `us-east5-a` and `europe-west4-b` actually have v5p, and only `us-east5-a` takes
    flex-start.

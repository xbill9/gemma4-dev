...
single-chip v5p flex-start Compute Engine instance. The main application is a single-file MCP server (`MCPServer`) in `server.py`. Its
...
- This rig runs one v5p chip (95 GiB HBM) via Compute Engine, so the tensor parallel size is `1`. The
  Cloud TPU API's 4-chip floor is what this rig exists to avoid. Older documentation and some
  Makefile examples incorrectly show `4`.
- Provisioning goes through `gcloud compute instances`, not the deprecated Cloud TPU API. The machine type
  is `ct5p-hightpu-1t-tpu`, the image is family `ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e` in project
  `ubuntu-os-accelerator-images`, and the provisioning model is an UPPERCASE enum
  (`FLEX_START`/`SPOT`/`STANDARD`). Use "v5p-1" in prose only,

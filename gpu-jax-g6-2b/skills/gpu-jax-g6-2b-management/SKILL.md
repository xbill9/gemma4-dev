...
`g6.xlarge` in the two runs recorded under `benchmarks/runs/` (an unrecorded spot-check
...
`g6.xlarge` is the default. `g5g.xlarge` is **supported** — it gets a 16 GiB swapfile,
...
- Instance discovery is scoped to `ManagedBy=gpu-jax-g6-2b`. Never operate outside that tag.

...
DEFAULT_SIZE = "g6.xlarge"
SWAP_SIZE = "g6.xlarge"       # 16 GiB host RAM -- the boundary case, inclusive
...
class G5gTopologyTests(unittest.TestCase):
    ...
    def test_every_size_is_supported_and_the_two_smallest_need_swap(self):
        ...
        Two different pressures, both measured, and 16 GiB is short for both:
          ...
          * g6.xlarge (16 GiB), 2026-08-26: mmaps fine, then `--ple-bits 8`
        ...
        """
        for size in (SWAP_SIZE, "g6.xlarge"):
        ...
        for size in ("g6.2xlarge", "g6.4xlarge", "g6.8xlarge", "g6.48xlarge"):
            with self.subTest(instance_type=size):
    ...
    def test_non_g6_rejected(self):
        for bad in ("g4dn.xlarge", "inf2.xlarge", "g6.unknown"):
...
class AdaConstraintTests(unittest.TestCase):
    """Ada (SM 8.9) HAS bf16 and fp8 -- the inverse of the Turing siblings.

    This class replaces `TuringConstraintTests`, which asserted the opposite and
    was inherited wholesale by the fork. Two of its four cases were still right
    on this chip and are kept; the dtype case is inverted, because bfloat16
    being the default is the entire premise of this rig.
    """

    def test_dtype_default_is_bfloat16(self):
        # Not a preference: `jax_e_model._compute_dtype()` returns bfloat16 at
        # SM >= 8.0 from the LIVE device, and this default states the same
        # thing so `tpu.env`, `server.py` and the device cannot drift apart.
        self.assertEqual(server.DTYPE, "bfloat16")

    def test_kv_cache_is_left_on_auto_not_forced_to_fp8(self):
        # Ada does have an fp8 datapath, so unlike the Turing siblings this is a
        # CHOICE rather than a hardware constraint. Nothing here has measured
        # fp8 KV, so the rig must not claim it by hardcoding the flag.
    ...
    def test_no_tensor_parallel_flag_is_emitted(self):
        ...
        argv = server._serve_argv(MODEL, "g6.12xlarge")
...
class QuantKnobTests(unittest.TestCase):
    ...
    def _argv(self, **env):
        ...
        for k, v in env.items():
            setattr(server, k, v)
        try:
            return server._serve_argv(server.MODEL_NAME, "g6.xlarge")
        finally:
...
class UserDataTests(BashSyntaxMixin, unittest.TestCase):
    ...
    def test_env_file_is_locked_down_before_the_token_lands(self):
        text = user_data()
        self.assertLess(text.index("chmod 600 /opt/jax-g6/env"), text.index("HF_TOKEN="))
    ...
    def test_swap_block_uses_only_portable_flags(self):
        ...
        /opt/jax-g6 and no install log.
        """
        rendered = user_data(instance_type="g6.xlarge")
    ...
    def test_swapfile_is_rendered_for_the_two_smallest_hosts(self):
        ...
        for size in ("g6.2xlarge", "g6.4xlarge", "g6.8xlarge"):
            with self.subTest(instance_type=size):
...
class DeploymentConfigTests(unittest.TestCase):
    ...
    def test_config_rejects_non_g6_only(self):
        # g6.xlarge is this rig's own default, so the rejection probe has to be a
        # genuinely wrong family -- g4dn is the sibling rig's T4 box.
        rejected = run(server.get_deployment_config(instance_type="g4dn.xlarge"))
...
class ObservabilityTests(unittest.TestCase, BashSyntaxMixin):
    ...
    def test_launch_reports_the_ami_it_booted(self):
        src = (ROOT / "server.py").read_text()
        body = src.split("async def create_g6_instance")[1].split("@mcp.tool")[0]

# CLAUDE.md — `gpu-jax-g6-2b`

Serving rig: **`google/gemma-4-E2B-it`** under **pure JAX** on **AWS EC2 G6** — an **x86_64**
host with an **NVIDIA L4** GPU (Ada, SM 8.9, **23034 MiB** measured, not the nominal 24 GB).
...
> **READ THIS BEFORE TRUSTING ANY NUMBER BELOW.** This rig was forked from
> `gpu-jax-g5g-2b` on 2026-08-28 and has **one measurement of its own**
> (`benchmarks/runs/2026-08-28-first-serve-g6/`). Most of this file is inherited T4G/Turing
> analysis. It is kept because the *mechanisms* are the same engine and it is genuinely
> useful history — but **Turing conclusions do not carry to Ada**, and the sections that are
> now superseded say so at the top. When in doubt, the run directory and `tpu.env` are this
> rig's evidence; anything else here is the sibling's until re-measured.

## This rig has served

**One run, its own**: `2026-08-28-first-serve-g6` on `g6.2xlarge`. It is the first serve, the
first sweep and the first xprof profile, all in one. The headline is **48.3–48.5 tok/s
single-stream**, flat across an 88x context range.

**That is 3.7x the T4G sibling's 12.80 at the identical configuration** — same build id
`51bc52c9e2e9`, same `ple4 + int8_lm_head`, and `tpu_jax_weight_bytes` byte-identical at
**6,155,450,950**. Only the chip differs, which is what makes it a clean control rather than a
comparison. See **The dtype tax is gone** below, and **Measurement** for the table.

Four things that still hold from the T4G lineage and are easy to lose:

- **Concurrency is not an axis and cannot be.** `MAX_NUM_SEQS=1`, `Gemma4EModelJAX` raises
  `NotImplementedError` for `B > 1`, and the decode step donates its KV buffers. Context ×
  output is swept; concurrency is closed. `tpu.env` marks values MEASURED or PREDICTED —
  respect the split and do not quote a PREDICTED number as a result.
- **Installability is not a served token.** Wheels resolving for a platform was never the same
  claim as "it serves". `verify_gpu_arch` remains the cheapest way to convert an install into
  evidence — it runs a real matmul on the device rather than checking that a flag was accepted.
- **Warm up before recording anything.** MEASURED here 2026-08-28: prefill at 2,057 tokens took
  **28,927 ms cold against 467 ms warm — 61x**, and decode 9.74 vs 48.3 tok/s. Faster silicon
  did **not** shrink this; the ratio grew, because warm decode got 3.7x faster while
  compilation did not.
- **`max_tokens` is part of the compiled shape.** `max_new_tokens` is a `static_argnames`
  entry, so `(bucket, max_tokens)` *is* the shape. Warming at a different `max_tokens` than you
  measure was a 4x error on the sibling.

## Why the hardware slot is `g6` and not `l4`

`NAMING.md` has the rule and it is now **general, not a carve-out**: on EC2 the hardware slot
is the **instance family** — `g5g`, `g4dn`, `g6`, `g6f`. Do not "correct" this to `l4`.

This rig was in fact created as `gpu-jax-l4-2b` and renamed the same day (2026-08-28), so the
`l4` reading has already been tried and rejected here. Three reasons it loses:

- **`g6f` exists and is half an L4.** A fractional-GPU family shipping the same SKU is exactly
  the case a SKU-based name cannot express: `gpu-jax-l4-2b` would be equally true of a `g6f`
  instance with half the device memory, and device memory is the binding constraint on this
  rig. `g6` vs `g6f` is a real distinction that `l4` erases.
- **The family is what you provision, quota and pay for.** You request a `g6.2xlarge`; you
  never request "an L4". Quota is per-family.
- **It pins the host architecture, which the SKU does not.** G6 is x86_64. This matters
  concretely — the fork from G5g flipped the host from Graviton2/aarch64, and the AMI path,
  the wheel tags and the DLAMI filter all had to change with it.

**The five `gpu-vllm-l4-*` artifact rigs predate the rule and keep `l4`.** They are not
evidence for the SKU reading and they are not renamed — they are artifact rigs whose hardware
was a bare "NVIDIA L4" with no recorded instance family. `NAMING.md` says this explicitly.

The chip is still called an **L4** everywhere it is the chip being discussed — in this file, in
`HARDWARE.md`, and in `tpu.env`. Only the slot is `g6`.

## The dtype tax is gone — the question this fork was opened to ask

**MEASURED 2026-08-28, `benchmarks/runs/2026-08-28-first-serve-g6/`.** This is the single most
important finding on this rig and it supersedes a large block of inherited analysis.

`gpu-jax-g5g-2b` spent days establishing that **87% of decode on Turing was dtype tax** —
54.0% dtype conversion plus 32.6% fp32 GEMV — and could not remove it. Three fixes were tried
on hardware there and all failed; the last one, converting the whole tree to float16, moved
throughput **+0.0%**, which left the cause formally open.

**On Ada it simply is not there.**

| | T4G (SM 7.5) | **L4 (SM 8.9)** |
| --- | ---: | ---: |
| decode, gauge | 12.80 tok/s | **48.3–48.5 tok/s** |
| total kernel time | 1466.0 ms | **362.8 ms** |
| dtype conversion | 54.0% | **0.0%** |
| fp32 `gemvx` | 32.8% | **zero such kernels** |
| TensorCore | 0.0% | 0.0% |
| peak HBM | 298.083 GiB/s | **279.441 GiB/s** |
| % of bandwidth roofline | 26% | **~100%** |

**The kernel table is a 1:1 substitution, and the call counts prove it.** The top three Turing
kernels were `wrapped_convert_1` (40 calls/step), `wrapped_convert_3` (20) and
`wrapped_convert_61` (1, the LM head). Here the top three are `gemm_fusion_dot_419` (**40**),
`gemm_fusion_dot_479` (**20**) and `gemm_fusion_dot_478` (**1**, the LM head). Same shapes,
same counts, same singleton. The conversion was not moved, fused away or hidden — **with a
native bf16 datapath it was never needed.**

The device picks this itself, which is the part worth keeping. `jax_e_model.py` reads the live
compute capability; the first line the process emits is

```
jax_e_model device policy: platform=gpu compute_capability=8.9 compute_dtype=bfloat16
```

No code change, no `tpu.env` change. On Turing the same line reads `compute_dtype=float16`.

### The regime changed, so the optimisation advice inverts

**The L4 has LOWER memory bandwidth than the T4G — 300 GB/s against 320 — and is 3.7x faster.**
That is the cleanest proof available that Turing decode was never bandwidth-bound.
...
Measured lands *just past* the naive floor, as expected — the `ple4` table is a **gather**, so
not all 6.155 GB streams per step. **This rig runs at its roofline where the T4G sat at 26% of
its own.**

**So: there is no overhead left to remove here.** Every "remove waste" item ranked in the
inherited **High-value optimizations** section is worth ~0 on this rig. Further decode speed
requires **fewer weight bytes** — a real int8 or int4 matmul. Ada has int8 tensor cores and
this path never touches them: `int8_lm_head` still dequantises the whole table to bf16 before
an ordinary matmul.

### The TensorCore reading is weaker here than on Turing — do not overclaim it
...
throughput. **But this is not the same quality of evidence as the Turing result.** There, the
kernel was literally cuBLAS `gemvx::kernel<int, int, float, float, float, float, ...>` — every
template parameter fp32, a direct signature. Here the kernels are XLA-generated
`gemm_fusion_dot` fusions, and xprof reporting them not-eligible may be a **reporting gap for
XLA's own fusions** rather than a fact about what the SM executed.

The `B=1` GEMV argument still applies and is still the likeliest explanation. But it does not
matter much either way: **the rig is bandwidth-bound, and tensor cores cannot help a
bandwidth-bound decode.** This is a curiosity here, not a lever.
...
## This rig IS an L4 now — the dtype policy inverted at the fork

The five `gpu-vllm-l4-*` rigs were written for **this exact silicon**, and the model port came
from a TPU rig. After the G5g → G6 fork, the L4 column below is no longer a sibling to avoid
copying from — **it is this rig.** The Turing column is what to stop copying from.

| | TPU v6e-1 | **this rig (SM 8.9)** | T4G sibling (SM 7.5) |
| --- | --- | --- | --- |
| compute dtype | `bfloat16` | **`bfloat16`** | `float16` |
| KV cache dtype | `bf16`/`fp8` | **`auto` → bfloat16**; fp8 available, untried | `auto` → float16 |
| device memory | 32 GiB HBM | **23034 MiB** | 15360 MiB |
| scoped memory | 16 MB VMEM | **99 KiB/block** | 64 KiB/block |
| matmul precision | `jax_default_matmul_precision=bfloat16` | **left at JAX's default** | left at JAX's default |
| fused W4A16 Pallas | yes | **untested — see below** | refused at startup |

Three things about the dtype policy that are easy to get wrong:

- **The device decides, not `tpu.env`.** `ports/gemma4/jax_e_model.py` reads the live compute
  capability and picks `bfloat16` at SM >= 8.0, `float16` below it. `DTYPE=bfloat16` in
  `tpu.env` is the *override* and now agrees with what the device picks;
  `JAX_E_COMPUTE_DTYPE` is the escape hatch. VERIFIED in the journal 2026-08-28.
- **The bfloat16 emulation trap is GONE, and that is the whole finding.** On Turing, bfloat16
  did not fail — it emulated through fp32 conversions, so numbers came out right while every
  matmul quietly paid 54% of decode. Ada has a native datapath, so `pre_ampere=false` and
  `_PRE_AMPERE_UNSUPPORTED` in `jax_engine.py` no longer applies to this hardware at all.
  **fp8 is available here** rather than refused — but see below before reaching for it.
- **`jax_default_matmul_precision` is still set only on TPU, and should stay that way.** The
  Turing reason (demoting fp32 inputs to a format with no unit) no longer applies, but nothing
  here has measured what setting it does on Ada. Do not reintroduce it speculatively.

**fp8 is newly available and is probably not the win it looks like.** Ada has an fp8 datapath,
so `resolve_cache_dtype()` will no longer raise. But KV is not the binding constraint on this
engine — the whole cache at 4K context is **~30 MiB** against a **~5 GiB** prefill transient,
a 174x gap — and decode is now bandwidth-bound on the **weights**, which fp8 KV does not touch.
`fp8_e5m2` remains **DEGRADED** for the same reason it was on the L4 artifact rigs: two mantissa
bits, visibly truncated output. If you want a memory win here it is weight encoding, not KV.

**The fused W4A16 Pallas kernel is untested here and may now fit.** It needs 550 KiB – 1.1 MiB
of scoped memory per block, against Turing's hard 64 KiB — three orders of magnitude out, which
is why `check_w4a16_fits_scoped_memory()` refuses at startup on the sibling. Ada raises the
per-block ceiling to ~99 KiB, which is **still far short**, so the refusal is expected to stand.
Nobody has run it. Do not assume either way without checking the guard's arithmetic on 8.9.
...
**`tpu.env` sets `INSTANCE_TYPE=g6.xlarge`, but the only size ever MEASURED here is
`g6.2xlarge`** (2026-08-28). **Every size is supported** — `_validate_instance_type` only
enforces the size list.

**The host-RAM arithmetic changed at the fork and the inherited analysis below no longer maps
onto the size names.** G6 has twice the host RAM of G5g at the same suffix: `g6.xlarge` is
**16 GiB** (where `g5g.xlarge` was 8) and `g6.2xlarge` is **32 GiB** (where `g5g.2xlarge` was
16). Device memory also went 15,360 → 23,034 MiB. So:

- **`g6.2xlarge` (32 GiB host) is above the swap gate and gets no swapfile.** That is the size
  that was measured, and it loaded and served without incident — so the whole swap mechanism
  below is, as of today, **unexercised on this rig.**
- **`g6.xlarge` (16 GiB host) sits exactly ON the gate** and is the one size that would render
  the swap block. **It has never been launched here.** Per the lesson recorded immediately
  below, that makes it untested code in the most literal sense.

`_user_data` provisions a swapfile at or below `_SWAP_AT_OR_BELOW_HOST_RAM_GB = 16`.

**The rest of this section is the T4G sibling's evidence.** It is kept because the *mechanism*
(load-time host-RAM pressure, and how it fails) is a property of the checkpoint and the loader,
not the GPU — but the sizes it names have different RAM here, so do not transfer a verdict
about `g5g.2xlarge` onto `g6.2xlarge`.

Two distinct pressures, both measured on G5g, and the larger one set the threshold:
...
written*, so the instance booted with an empty `/opt/jax-g6` and no install log at all. It
...
**`g6.12xlarge`, `g6.24xlarge` and `g6.48xlarge` carry 4, 4 and 8 L4s.** (Note this differs
from G5g, where only `16xlarge`/`metal` were multi-GPU, and `g6.16xlarge` is **single**-GPU
despite the suffix — do not infer GPU count from the size name.) **Nothing shards across them.**
The engine is single-device (`jax.devices()[0]`), `_serve_argv` emits no tensor-parallel flag,
and every GPU but the first idles. `_tensor_parallel_size` reports the GPU count and nothing
acts on it yet. A bigger instance buys host RAM and vCPUs, **not** device memory.
...
The unit is `jax-g6.service` — read it with `journalctl`, not `docker logs`.
...
- Scope instance discovery to `ManagedBy=gpu-jax-g6-2b`. Unlike the inf2 rig, which keeps a
...
`gpu-jax-g6-2b`, which prefixes every tool as `mcp__gpu-jax-g6-2b__…`. **All four agree as of
2026-08-28** — they did not before. A mismatch makes `/mcp` and the tool prefix disagree about
what this rig is.

**The G5g → G6 fork broke this again, in a new and worse way. Found and fixed 2026-08-28.**
`.mcp.json`, `plugin.json` and `settings.local.json` had been updated; **`.codex/config.toml`
was untouched and wrong in three independent ways at once:**

1. it registered the server as `gpu-jax-g5g-2b`, disagreeing with the other three;
2. it pointed at `skills/gpu-jax-g5g-2b-management/mcp/server.py`, **a path that does not
   exist here**; and
3. **the dangerous one** — its approval gates named `create_g5g_instance`,
   `terminate_g5g_instance` and `stop_g5g_instance`. This rig's tools are `*_g6_*`. **The
   gates matched nothing, so every destructive tool was ungated under Codex while appearing
   to be gated.**

**A gate on a tool name that does not exist fails open and says nothing.** That is the lesson
worth keeping, and it generalises past this file: a rename silently converts a safety control
into a no-op, and nothing tests it. `deploy_jax_server` was added to the gated set at the same
time. Ground truth for tool names is always `grep -n "^@mcp.tool" server.py`.

**`plugin.json` was also stale in content though correct in name** — its description said
"AWS EC2 G5g (Graviton2 + NVIDIA T4G)" and its keywords were `graviton2`/`t4g`/`turing`/
`aarch64`, i.e. it advertised the wrong host architecture *and* the wrong GPU. Corrected.

**The monorepo-root marketplace had no entry for this rig at all** and gained one on
2026-08-28. That is the copy `/plugin` actually reads.
...
`server.py` was right throughout (`RIG_NAME = "gpu-jax-g6-2b"`, asserted by
`test_rig_name_matches_directory`) — which is why the breakage was invisible to the tests, and
why it recurred at the next fork. **The test pins `server.py` against the directory name and
nothing pins the other four files against either.**
...
and it gained a `gpu-jax-g6-2b` entry on 2026-08-28.
...
> **RESOLVED BY THE FORK — 2026-08-28.** The problem this section chases is a **Turing**
> problem: the loader stores bf16 while `COMPUTE_DTYPE` is float16, so XLA converts in front of
> every use. On Ada `COMPUTE_DTYPE` **is** bfloat16, so there is no mismatch and no conversion —
> MEASURED at 0.0% of kernel time with zero `wrapped_convert_*` kernels. **Do not implement any
> of the fixes discussed below on this rig; there is nothing here to fix.** The file
> `docs/bf16-weights-on-turing.md` is the sibling's evidence, kept for the mechanism.

**`docs/bf16-weights-on-turing.md` — measured 2026-08-24 on the T4G. Root cause confirmed,
never fixed there; removed here by having a native datapath instead.**
...
- **Build id matched end to end** — deploy reported `6852f5680f43` and the
  payload root it resolved, `PAYLOAD_SHA` landed in `/opt/jax-g6/app`, and
...
> **SUPERSEDED ON THIS RIG — 2026-08-28.** This section measures a **T4G**, and it was the
> reason this rig was forked. On Ada the conversion share is **0.0%** and the fp32 GEMV
> kernels do not exist; decode is 3.7x faster and now sits at the bandwidth roofline. See
> **The dtype tax is gone** at the top of this file. Keep reading only for the mechanism and
> the method — the *numbers* below describe `gpu-jax-g5g-2b`, not this rig.

**`benchmarks/runs/2026-08-25-context-sweep-g5g/` — MEASURED on the T4G sibling, with the first
xprof profile taken on that rig.** This was the single most important number there and it
subsumed most optimization ideas you might arrive at independently.
...
## `MAX_MODEL_LEN` is 4096 because 8192 was not reachable — on a T4G

> **INHERITED AND NOW UNTESTED — 2026-08-28.** Every number below was measured against
> **15,360 MiB**. This card has **23,034 MiB** (1.5x) and the first-serve run peaked at
> **6.197 GB**, so 4096 is an assumption here, not a ceiling, and it is probably too low.
> MEASURED here: 3,593 tokens serves at full speed; a 4,105-token prompt returns 500 with
> `ValueError: Prompt of 4105 tokens leaves no room within max_model_len=4096` — **that is
> this config limit refusing the request, not an OOM.** Do not cite it as a hardware result.
> **Finding the real ceiling on 23 GB is the obvious next run.** The value was deliberately
> left at the conservative inherited number rather than raised on a guess.

**MEASURED 2026-08-26 on the T4G sibling.** Lowered from 8192 there, and the old value was not
an honest number — a little over half of it was reachable.
...
- **`requirements-profiling.txt` was never on the instance.** It is deliberately excluded from
  ...
  operators to `pip install -r /opt/jax-g6/requirements-profiling.txt`, **a path that has
...
> **THE RANKING BELOW IS THE T4G's AND IS NOT THIS RIG'S — 2026-08-28.** Items 1–3 all rank
> *overhead removal*, and this rig has no overhead left to remove: it measures **49.5 tok/s
> against a 48.8 tok/s weight-streaming floor**, i.e. at the wall, where the T4G sat at 26% of
> its own ceiling. **On Ada the ranking collapses to one item: fewer weight bytes.** A real
> int8 matmul (item 5 here, and Ada has int8 tensor cores this path never touches) is the only
> entry with headroom behind it. Items 1 and 2 are worth ~0 — the fp32 promotion they describe
> does not happen on this hardware. Keep the section for its method: **always check a proposed
> win against the physical envelope before doing the work.**

**Derived 2026-08-28 from `benchmarks/runs/2026-08-27-baseline-xprof-g5g/`, a T4G run.** Ranked
by measured upside, not by plausibility. The whole point of the ordering was that **items 1 and
2 are the same bug**, and everything below them is rounding error until that bug is gone.
...
**This rig has ONE measurement of its own**, in `benchmarks/runs/<date>-<what>-g6/` where
`<hw-short>` equals the hardware slot:

| Run | Decode | What it added |
| --- | ---: | --- |
| `2026-08-28-first-serve-g6` | **48.3–48.5 tok/s** | First serve, first sweep and first xprof, in one. Conversion 54.0% → **0.0%**; at the bandwidth roofline. |

**`benchmarks/runs/` carries NO OTHER RUNS.** The eight `*-g5g` runs referenced throughout this
file live in `gpu-jax-g5g-2b`, not here — they were deliberately not copied at the fork, so
`benchmarks/rollup.py` cannot count another rig's results against this one. When this file
cites a `-g5g` run, it is citing **the sibling**.

**The `-g6` suffix is load-bearing and was briefly wrong.** `tune_loop.py` hardcoded `-g5g` on
the run directory, inherited from the fork, and filed this L4 result under the T4G's hardware
short name — the misattribution the monorepo `CLAUDE.md` names explicitly ("a report's
`<hw-short>` is the hardware *measured*"). Fixed 2026-08-28 to derive from `RIG_NAME`'s
hardware slot. **Never reintroduce a literal.**

Quote the **`tpu_jax_decode_tokens_per_second` gauge**, not end-to-end tok/s: end-to-end
includes prefill and the HTTP round trip, so it falls with context (46.23 → 27.55 across the
sweep) while decode does not move (48.5 → 48.3). The two disagree by design.
...
- **12.8 / 13.1 tok/s** — `gpu-jax-g5g-2b`, the T4G sibling. **Same engine, same build id, same
  weight bytes, different chip.** This is the most dangerous one precisely because everything
  else matches: it is the *control* this rig is measured against, not a baseline it inherits.
  Cite it only as a comparison and always name the chip.
- **43.1 / 44.24 tok/s** — the vLLM sibling on `g5g.4xlarge` / `g5g.xlarge`, 2026-08-12/13.
  T4G, different runtime, and obtained *with reduced Triton tiles*. Note this rig has now
  **passed** it (48.3–48.5) — but on different silicon, so that is not a runtime comparison.
  The runtime comparison is `gpu-vllm-g6-2b`, which is what that rig exists for.
...
- **Anything from `~/gemma4-tips`** — that tree duplicated its own artifacts and its directory
  names misattribute both model and chip. Never read a model or a chip off one. **This applies
  with special force to the five `gpu-vllm-l4-*` artifact rigs, which are the same GPU as this
  rig** — same chip is not same measurement, and their provenance is the weakest in the tree.

A config flag being accepted is not evidence it did anything. Cross-check against an absolute
physical bound — **300 GB/s of GDDR6 and 23034 MiB is the whole envelope here** (the T4G's was
320 GB/s and 15360 MiB) — not against another config.

...
g6.xlarge across the two runs under benchmarks/runs/ (a spot-check on
...
SERVICE_NAME = os.getenv("SERVICE_NAME", "jax-g6")
...
# Ada (SM 8.9) has a native bfloat16 datapath, so `_compute_dtype()` returns
# bfloat16 at SM >= 8.0 and this default states the same thing. On the Turing
# siblings it is float16; the device still decides at runtime either way.
...
# g6 topology: (GPUs, host RAM GiB). Read from `describe_instance_types` on
# 2026-08-28, not from a product page. As with g4dn the ladder is not monotonic:
# 12xlarge and 24xlarge carry four L4s, 16xlarge carries one.
_G6_SIZES = {
    "g6.xlarge": (1, 16),
    "g6.2xlarge": (1, 32),
    "g6.4xlarge": (1, 64),
...
# Host RAM at or below this gets a swapfile. INHERITED, NOT MEASURED HERE --
# both figures come from `gpu-jax-g5g-2b`, whose 2026-08-26 run was OOM-killed
# five times at 14.3 GB anon-rss because quantize_ple_table upcasts the 4.70 GB
# PLE table to float32 while the full parameter tree is still resident. That is
# a host-RAM pressure and nothing about it is GPU-specific, so it should
# transfer -- but it has not been reproduced on this rig, and the default size
# here has exactly 16 GiB, which is the boundary case.
...
def _needs_swap(instance_type: str) -> bool:
    ...
    Two distinct pressures, both real, and the larger one decides:
      ...
      * g6.xlarge (16 GiB) mmaps fine and then dies in quantize_ple_table,
...
def _user_data(model: str, instance_type: str) -> str:
  ...
  # making the swap threshold inclusive rendered this block for g6.xlarge --
...
async def list_g6_instances() -> str:
    """List instances tagged ManagedBy=gpu-jax-g6-2b."""
    try:
...
async def check_g6_quotas() -> str:
    ...
    G-family types, counted in vCPUs — a g6.xlarge needs 8.
    """
    try:
...
**This rig has served, and the numbers are its own.** 12.4-12.5 tok/s decode on
`g6.xlarge` in the two runs recorded under `benchmarks/runs/`; an unrecorded

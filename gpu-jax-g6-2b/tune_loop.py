...
S3_RESULTS = "s3://vllm-models-bucket/benchmarks/gpu-jax-g6-2b"
REMOTE_OUT = "/opt/jax-g6/loopout"
FILLER = "token "
...
mkdir -p {out}
cd /opt/jax-g6/app
set -a; . /opt/jax-g6/env; set +a
systemctl stop jax-g6
...
PYTHONPATH=/opt/jax-g6/app python3.14 - <<'XP' >__OUT__/xprof_extract.log 2>&1 || true
import glob, json
...
async def run(args) -> None:
    ...
    # The <hw-short> suffix is the hardware MEASURED, not the rig that hosts the
    # harness -- so derive it from RIG_NAME's hardware slot rather than hardcoding.
    # It arrived from the g5g fork as a literal "-g5g" and would have filed this
    # rig's L4 results under the T4G's name, which is the misattribution the
    # monorepo CLAUDE.md calls out by name.
    hw_short = server.RIG_NAME.split("-")[2]
    ...
    if not args.no_profile:
        ...
        # `pip install -r /opt/jax-g6/requirements-profiling.txt` referenced a

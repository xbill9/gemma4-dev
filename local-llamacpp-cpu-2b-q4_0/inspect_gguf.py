...
This exists because sizing a rig from the file size on disk gives the wrong
answer, and confidently so. The file is 3.35 GB; the part llama.cpp touches on
every token is 1.31 GiB.
...
of this file. On the GPU sibling that decided whether the model fit the card; on
this CPU-only rig it decides how much of the file has to stay hot in the page
cache, because only a few of its rows are read per token.
...
def main() -> int:
    ...
    for nb, name, ty, shape in rows[:6]:
        mark = "  <- LAZY, rows read from the mmap" if name in LAZY_TENSORS else ""
        print(f"    {nb / 1e6:8.1f} MB  {name:<28} {ty:<7} {shape}{mark}")
    print()
    print(f"  lazy (GET_ROWS from mmap):  {lazy / 1e9:.3f} GB  ({100 * lazy / total:.0f}% of file)")
    print(f"  touched every token:        {resident / 1e9:.3f} GB = {resident / 2**30:.2f} GiB")

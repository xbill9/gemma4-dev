...
- **Label coverage.** The read scores a label outside the top 32 at a floor, and arms differ in how often that happens (for example E2B emotion: every label returned on 82.7% of bf16 reads, 100% of 4-bit reads). `quant_compare.py` now reports, per group, that share for each arm and the paired difference on only the records where both arms returned every label. For the four tasks those differences move by at most 0.011 from the full-record ones (E2B, AG News). The suite reads did not record the count; `nimble_suite/run_suite.py` now does (one added field, `labels_returned`, as `run_eval.py` records for the tasks), so it differs from `../jev-tpu`'s copy from here on.

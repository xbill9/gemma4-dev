...
| `~/gemma4-dev/gpu-vllm-t4-2b` | **GCE VM with the GPU already attached** (`n1-standard-2`, `us-west2-b`) — nothing provisions it | vLLM, pip-installed | t4 (TU104 + Turing SM 7.5, 15360 MiB) | `gemma-4-E2B-it` | — — added 2026-09-17. **Slot 1 is `gpu` and NOT `local`, and this is the entry to read before re-opening that.** Nothing here provisions, waits for or releases the accelerator, which is the `local` section's own test — but the slot is decided by WHO OWNS THE MACHINE, and this is a Compute Engine VM: "SSH into a cloud VM you provisioned and it keeps that VM's platform value." So it is the **first `gpu` rig with no provisioning half**, a new combination rather than a new value, and it is not a precedent for calling a cloud VM `local`. **Slot 3 is `t4`, the SKU, and the EC2 family rule does not reach it** — that rule is explicitly "on EC2", where the family is what you provision and pays for the host architecture; this is Compute Engine, so the SKU applies as it does everywhere else. Do not "correct" it to `g4dn` (that is the EC2 sibling) and do not invent an `n1` slot. `t4` collides with nothing: the collision argument that helped earn the EC2 carve-out was about `t4g`, which AWS also sells as GPU-less Graviton2 CPU instances. **Slots 1 and 3 are the only slots that differ from `gpu-vllm-g4dn-2b`**, and slot 3 differs only in spelling, so the pair is a control-plane A/B on identical silicon — the same shape as `local-llamacpp-1650ti-2b-q4_0` against `gpu-llamacpp-g5g-2b-q4_0`. Serving flags carried verbatim from the twin to keep that comparable. **Nothing installed, nothing served, nothing measured** |
| `~/gemma4-dev/gpu-vllm-mi300x-2b` | **DigitalOcean GPU droplet** (`gpu-mi300x1-192gb-devcloud`, via AMD Developer Cloud) | vLLM in Docker | mi300x (AMD Instinct MI300X VF, CDNA 3, gfx942) | `gemma-4-E2B-it` | — — added 2026-09-16, **the first AMD rig and the first on a DigitalOcean control plane**. Slot 1 is `gpu` under the existing definition — a general-purpose GPU attached to a VM, any cloud — so **no new platform value was added**, and the cloud stays out of the name as the scheme intends. **Slot 3 is the chip, not the size slug**, and that is not the EC2 rule being broken: the EC2 rule names the instance family because AWS sells families that mix hosts and chips, while `gpu-mi300x1-192gb-devcloud` already *is* the chip plus its count and memory, so the family form would only add the vendor's packaging. `mi300x` also leaves room for an `mi325x` or `mi355x` sibling to differ in exactly one slot. No encoding slot: these are reference bf16 weights |
...
| `~/gemma4-dev/local-ollama-1650ti-2b-q4_0` | **Local workstation** — no control plane, nothing provisioned | Ollama daemon (links `libllama.so`) | 1650ti (TU117, sm_75, 4096 MiB) | `gemma4:e2b-it-qat` | `q4_0` — added 2026-09-04, the **second `local` rig**, forked from `local-llamacpp-1650ti-2b-q4_0`. **Slot 2 is the only slot that differs from it**, which makes the pair the sharpest `llamacpp`-vs-`ollama` test in the tree: same card, same weights, same engine, so everything measurable is a choice the daemon makes. **Slot 4 is `2b` even though `ollama show` says `4.6B`** — the model slot names the checkpoint (E2B), not the daemon's parameter count, exactly as in every other E2B rig. **Slot 5 is a claim about what Ollama says it packaged**, not about a file this rig can hash: the tag is Google's QAT GGUF re-containered with the embedded chat template stripped (3,349,514,112 B against Google's 3,349,516,256, different digest), confirmed here on x86_64 against `gpu-ollama-g5g-2b-q4_0`'s arm64 reading. **First light and two sweeps 2026-09-04**: end-to-end throughput ties the llama.cpp sibling within 1% in all ten context cells and decode is ~3% behind once corrected for chunk batching. The stock tag holds **2762 MiB of 4096 against the sibling's 1618**, because the daemon loads the 986 MB vision/audio projector unconditionally and `ollama ps` does not count it — so `MODEL_NAME` is `gemma4:e2b-it-qat-text`, the same blob with that layer dropped, at **1612 MiB**. **Slot 5 survives the change**: it is still Ollama's own re-containered artifact by digest, minus a layer, not Google's file substituted in. The projector also carried a **2048-token minimum per slot that applies to multimodal models only**, which is what stopped the stock tag holding 32 slots on this card; projector-free it reaches the sibling's exact geometry and the two agree within 4% through c=8 |
| `~/gemma4-dev/local-llamacpp-cpu-2b-q4_0` | **Local workstation** — no control plane, nothing provisioned | `llama-server`, driven directly, `GGML_CUDA=OFF` build | **`cpu`** — no accelerator (i7-10750H, 6C/12T, AVX2 — no AVX-VNNI, no AVX-512) | `gemma-4-E2B-it-qat-q4_0-gguf` | `q4_0` — retargeted 2026-09-15. **Until that date the directory was a byte-identical copy of `local-llamacpp-1650ti-2b-q4_0`**, `-ngl 99` and `nvidia-smi` included. **Slot 3 is the only slot that differs from that sibling**, so the pair is a CPU-vs-GPU A/B on identical weights through the identical runtime — but only once both are measured on the same host, and the 1650 Ti is not on the machine this rig was retargeted on. CPU only is **enforced rather than configured**: `-ngl 0` and an empty `CUDA_VISIBLE_DEVICES` are hardcoded and the serve tool refuses a GPU build. Serving as of 2026-09-16 and rebuilt on `f95b0d9` 2026-09-22. **The host CPU in this row was wrong until 2026-09-22** — it read i7-1360P with a hybrid 4P+8E topology; the machine is a homogeneous 6-core i7-10750H (Lenovo Yoga 9 15IMH5), and the 1650 Ti **is** on it, so the A/B pair is on one host. The 18-cell thread/affinity sweep was interpreted through the wrong die and is quarantined; `KV_CACHE_TYPE` and `FLASH_ATTENTION` remain UNMEASURED |
| `~/gemma4-dev/local-vllm-cpu-2b` | **Local workstation** — no control plane, nothing provisioned | vLLM, CPU backend | **`cpu`** — no accelerator | `gemma-4-E2B-it` | — — retargeted 2026-09-04. **Until that date the directory was a verbatim copy of `gpu-jax-g4dn-2b`**: 58 tracked files describing an AWS G4dn with a T4, carrying `jax_engine.py`, `boto3` and a skill named `gpu-jax-g4dn-2b-management` — so three of the four slots were claims nothing in it supported, and the skill name is the destructive kind (`make skill-install` `rm -rf`s its destination). **Slot 3 is `cpu` per the slot-3 rule for a host with no accelerator.** It has served nothing and the budget is **close rather than closed**: the w4a16 checkpoint is 8.32 GB (MEASURED), needing 10.46 GB against ~9.3 GB free on a 16.42 GB machine — short by ~1.2 GB of headroom, not of hardware. Three claims in the first version of the rig were assumptions written as measurements and all three ran pessimistic: there is **no fp32 upcast** on x86 (`CpuPlatform.supported_dtypes` returns bf16 unconditionally), **AVX2 is a first-class build target** in vLLM's CPU cmake, and **w4a16 saves 19% not 75%** because the checkpoint's `ignore` list holds the vision tower and embeddings at bf16. `start_vllm_server` still refuses while over budget — exceeding host RAM is *accepted* and paid for in swap, so an over-budget serve is indistinguishable from a loading one. **The reachable CPU baseline on this machine is `local-jax-cpu-2b`**, whose `PLE_BITS=4` takes the weights to 5.752 GB; vLLM has no equivalent lever |

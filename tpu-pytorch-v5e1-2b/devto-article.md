...
2. A **Claude Code skill** (`tpu-pytorch-v5e1-2b-management`) that teaches Claude *when* and *how* to use those tools well.
...
For `tpu-pytorch-v5e1-2b-management`, the skill encodes things like:
...
This copies the skill into `<project>/.claude/skills/tpu-pytorch-v5e1-2b-management/` and merges the `tpu-devops` entry into that project's `.mcp.json` without touching your other servers. Restart Claude Code in the target project, approve the server, done.
...
- The **skill is active inside its own repository** — open Claude Code in a clone and the `tpu-pytorch-v5e1-2b-management` skill and `tpu-devops` server are already wired up, so every development session doubles as an integration test.

...
def main() -> int:
    ...
    ap.add_argument("--rig", default="gpu-ollama-g5g-2b-q4_0")
    ...
    # NO metrics.prom ON THIS RIG. Ollama exposes no /metrics endpoint at all,
    # so a sweep here never writes one -- reading it unconditionally, as the
    # llama.cpp sibling does, would crash every report with FileNotFoundError.
    # The timings this rig has live in sweep.json, from the client-side stream.
    ...
    version = args.engine_version or "ollama (pass --engine-version with the BUILD_ID)"
    max_len = args.max_model_len or health.get("seq") or 0
    weights_gib = args.weights_gib or round(
        (health.get("weight_bytes") or 0) / 2**30, 3)
    ...
    report = {
        ...
        "run": {
            ...
            "date": run_id[:10],
            "operator": "xbill",
            "source": (f"gpu-ollama-g5g-2b-q4_0/sweep.py + make_report.py; "
        ...
        "throughput": {
            "workload": {
                "tool": f"gpu-ollama-g5g-2b-q4_0/sweep.py (decode-source "
    ...
    report["notes"].append(
        ...
        "/api/generate's eval_count/eval_duration, which time decode alone. Both are in sweep.json "
        "and they do not agree."
    )
    # Ollama exposes NO degeneracy counter -- and no /metrics at all. So the note is emitted only when the
    ...
    if degen is None:
        report["notes"].append(
            "Degeneracy was NOT counted server-side: Ollama exposes no metrics "
            "endpoint at all. verify_model_health applies a local heuristic per "
            "probe; this run has no whole-run figure."
        )
    else:

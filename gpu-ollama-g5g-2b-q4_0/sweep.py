...
rather than a degraded one.** `usage.decode_tokens_per_second` is an invention of
our own JAX and PyTorch servers; Ollama's OpenAI shim does not emit it, so the
probe falls through and the harness measures the client-side inter-token
statistic every rig can produce. That is exactly the common statistic the
2026-08-31 rework exists to provide.

**And here there is no server-side alternative to fall back on.** Ollama exposes
no `/metrics` endpoint at all -- verified against `server/routes.go`. The MCP
`get_metrics` tool PROBES instead, running one generate and reading
`eval_count`/`eval_duration` (nanoseconds) off the response. That is a
single-request measurement and cannot describe a sweep, so use it to sanity-check
a run, never as the run's number.

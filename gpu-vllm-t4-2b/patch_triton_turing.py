...
WHY A SCRIPT AND NOT A BUILD, AND WHY NOT EVEN AN IMAGE. Three rigs now deliver
this same clamp three different ways, cheapest last:

  gpu-vllm-g5g-2b   aarch64: no SM 7.5 kernels published at all, so vLLM is
                    compiled from source (~67 min) and the patch rides along.
  gpu-vllm-g4dn-2b  x86_64 docker: the amd64 image already carries 7.5, so a
                    derived image is built `FROM` it with one COPY. Seconds.
  gpu-vllm-t4-2b    x86_64 pip, THIS RIG: there is no image. vLLM is installed
                    into the system python and the target is the installed
                    module in site-packages. Milliseconds.

THE CHEAPEST DELIVERY IS ALSO THE LEAST CONTAINED, and that is the one thing to
carry away. The EC2 siblings edit a file inside an image they built and throw
away; a bad patch costs a rebuild. Here the file is the host's own vLLM, shared
with every other process and every other rig on the box, and `pip install -U
vllm` silently reverts it. Re-run `verify_turing_patch` after ANY vLLM upgrade.
...
# The rig name is in the marker on purpose: an operator grepping the file for
# "why is this different from upstream" gets an answer that names the rig.
#
# THE IDEMPOTENCY CHECK KEYS ON `SENTINEL`, NOT ON `MARKER`, and on this rig that
# distinction is load-bearing where it was cosmetic on the EC2 siblings. They
# patch a file inside a throwaway docker image, one rig per image. Here the
# target is SYSTEM site-packages, shared by everything on the host — so a marker
# carrying only this rig's name would fail to recognise a clamp that a sibling
# script had already inserted, and the file would be patched TWICE: two `if
...
MARKER = "# gpu-vllm-t4-2b: Turing shared-memory clamp"
...
def main(argv: "list[str]") -> int:
    ...
    if SENTINEL in text:
        who = MARKER if MARKER in text else "a sibling rig's copy of this script"
        print(f"patch_triton_turing: already patched by {who}: {path}")
        return 0
    if check_only:

...
The inference server is deployed on **Cloud TPU v5p (one chip, via Compute Engine)** using the `vllm-tpu` specialized container.

*   **Hardware:** 
    *   **TPU Version:** v5p (Compute Engine, not the deprecated Cloud TPU API)
    *   **Machine type:** `ct5p-hightpu-1t-tpu` (1 chip, 95 GiB HBM)
*   **Software:**
...
### 1. Create the TPU v5p Instance
```bash
gcloud compute instances create vllm-gemma4-tpu \
    --machine-type ct5p-hightpu-1t-tpu \
    --image-family ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e \
    --image-project ubuntu-os-accelerator-images \
    --boot-disk-size 200GB --scopes https://www.googleapis.com/auth/cloud-platform \

...
> ## ⚠️ Inherited from the v5e-1 rig — every number below was measured on different hardware
>
> This rig became **v5p (4 chips, 95 GiB HBM each)** on 2026-08-10. This file arrived unchanged with the
> fork from `tpu-vllm-v5e1-2b` and describes a **single 16 GiB v5e-1 chip at `TENSOR_PARALLEL_SIZE=1`**.
> The linked report lives in that rig, not this one:
...
>
> Anything memory-bound in here — KV pool sizing, `--gpu-memory-utilization`, the feasible context ×
> concurrency envelope — is decided by a constraint v5p does not have, with ~24x the HBM per chip and four
> of them. Nothing here has been re-measured on v5p. **Treat it as a prior to test, not a configuration to
> apply**, and note that this file's own history is a warning: three of its first-version recommendations
> were reversed by measurement.


**Status: measured 2026-08-09.** This file states what the rig should run. The evidence lives in
[`benchmarks/runs/2026-08-09-serving-params-v5e1/REPORT.md`](benchmarks/runs/2026-08-09-serving-params-v5e1/REPORT.md)
— nine arms on a spot `v5litepod-1`, 36 benchmark cell-runs, and the functional tests. Read that for
numbers; read this for the decision.

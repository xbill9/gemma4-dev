...
**bf16**
- native in the MXU?: ✅
- what it buys on v5e: the baseline — everything here runs in it

**int8**
- native in the MXU?: ✅ **2x bf16 throughput**
- what it buys on v5e: the *only* low-precision compute win

**fp8**
- native in the MXU?: ❌
- what it buys on v5e: storage and bandwidth only — values widen back to bf16 before the matmul

**int4 / fp4**
- native in the MXU?: ❌
- what it buys on v5e: footprint and bandwidth only, then unpack to bf16
...
**KV cache, bf16 (`auto`)**
- status on this build: ✅ the only one worth running

**KV cache `fp8_e4m3` / `fp8_e5m2`**
- status on this build: reachable, **1.000x capacity** — the block layout is word-aligned, so narrowing the element buys padding, not room. ~2% slower

**KV cache `int8`**
- status on this build: ❌ **rejected by the CLI enum** — never reaches the engine

**KV cache `int8_per_token_head`, `turboquant_*`, `nvfp4`, `fp8_inc`, `fp8_ds_mla`**
- status on this build: accepted by the CLI, then **kill the server at boot**

**Weights, compressed-tensors w4a16 (Google's QAT format)**
- status on this build: ❌ `NotImplementedError` on the JAX path

**Weights, mxfp4**
- status on this build: ❌ MoE-only; E2B is dense, so there is nothing to attach to

**Weights, qwix PTQ int8/int4**
- status on this build: ❌ does not boot — the concrete path OOMs on quantization temporaries, the abstract path raises binding weights

**Weights, AWQ / GGUF / q4_0**
- status on this build: ❌ torch path or absent
...
- default: `False`
- what it does: precompile attention at power-of-two *request* buckets instead of a single shape at `max_num_seqs`

**`SLICE_ROPE_CACHE`**
- default: `False`
- what it does: slice the rotary cache to `max_model_len` at load — free HBM

**`NUM_PRECOMPILE_WORKERS`**
- default: `1`
- what it does: parallel XLA precompilation; compile is 685 s of an 857 s boot

**`VLLM_TPU_BUCKET_PADDING_GAP`**
- default: `0`
- what it does: linear bucket increments (use 128) instead of powers of two — the fix for the 2496-pads-to-4096 problem

**`VLLM_XLA_CHECK_RECOMPILATION`**
- default: `False`
- what it does: error on a runtime recompile; turn on for one validation boot

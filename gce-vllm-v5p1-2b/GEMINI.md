...
*   `MCP_SERVER_NAME`: Name this server advertises, and the key it must be registered under — it prefixes
    every tool as `mcp__<name>__find_tpu` (defaults to the rig directory name, `tpu-vllm-v5p1-2b`).
...
Now, every time you run `gemini`, it will be powered by your private TPU v5p instance.
...
*   **`get_zones_with_available_quota`**: Gets GCP zones that have available TPU quota for TPU v5p.
*   **`find_tpu`**: Scans multiple zones sequentially to find and provision a TPU v5p resource.
*   **`find_gpu`**: Scans the GCP project for available GPU resources (GCE VMs, Cloud Run services, and zones with quota).
*   **`get_vllm_deployment_config`**: Generates the `gcloud` command for a single-host TPU v5p vLLM deployment.

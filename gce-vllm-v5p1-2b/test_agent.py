...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_vllm_deployment_config(self, mock_run_command, mock_get_secret):
        ...
        self.assertIn("--machine-type=ct5p-hightpu-1t-tpu", config)
...
class TestProvisioningModel(unittest.IsolatedAsyncioTestCase):
    """Covers the flex-start / spot / on-demand split on the Compute Engine path.

    Compute Engine spells the model as an UPPERCASE enum, where the Cloud TPU API took
    lowercase `flex-start` and a bare `--spot` switch. A lowercase value here is silently
    wrong, so these assert the exact casing.
    """
    ...
    def test_flex_start_flags_bound_the_run(self):
        ...
        # Compute Engine requires a termination action with flex-start; the TPU API had none.
        self.assertIn("--instance-termination-action=DELETE", flags)
        self.assertNotIn("--provisioning-model=SPOT", flags)
    ...
    def test_on_demand_is_standard_with_no_run_bound(self):
        flags = _provisioning_flags("on-demand")
        self.assertEqual(flags, ["--provisioning-model=STANDARD", "--labels=purpose=on-demand"])
        # No automatic stop: on-demand bills until someone destroys it.
        self.assertNotIn("--max-run-duration=4h", flags)
    ...
    async def test_create_passes_spot_flag_to_gcloud(self, mock_run_command, mock_get_secret):
        ...
        ok, msg = await _create_tpu_instance("qr-test", "us-east5-a", "spot")

        self.assertTrue(ok, msg)
        cmd = mock_run_command.call_args[0][0]
        self.assertIn("--provisioning-model=SPOT", cmd)
        self.assertNotIn("--max-run-duration=4h", cmd)
        self.assertIn("--machine-type=ct5p-hightpu-1t-tpu", cmd)
        self.assertIn("--image-family=ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e", cmd)
        # Without cloud-platform scope the boot-time Secret Manager fetch fails 30 min in.
        self.assertIn("--scopes=https://www.googleapis.com/auth/cloud-platform", cmd)
    ...
    async def test_create_defaults_to_flex_start(self, mock_run_command, mock_get_secret):
        ...
        ok, _ = await _create_tpu_instance("qr-test", "us-east5-a")
    ...
    async def test_create_rejects_unknown_model_before_calling_gcloud(self, mock_run_command, mock_get_secret):
        ok, msg = await _create_tpu_instance("qr-test", "us-east5-a", "preemptible")
...
def _sku(description, usage_type, price, region="us-west4"):
...
# Shapes and rates copied from the live catalog for us-west4 on 2026-08-06. The decoys are
# the real reason this matcher needs testing: four other SKUs describe the same chip in the
# same region, and three of them share the OnDemand usage type.
# These stay v5e on purpose even though the rig is v5p: they are fixtures for the matcher's
# decoy rejection, not a claim about this rig's hardware. The v5p catalog has the identical
# shape (`TpuV5p running in Columbus` / `DWS Defined Duration V5p ...` / `... Spot Preemptible
# VMs ...` in us-east5, confirmed live 2026-08-10), so the same patterns cover both.
FAKE_SKUS = [
    _sku("TpuV5e running in Las Vegas", "OnDemand", 1.20),
    _sku("DWS Defined Duration V5e running in Las Vegas", "OnDemand", 0.60),
    _sku("TpuV5e attached to Spot Preemptible VMs running in Las Vegas", "Preemptible", 0.5779),
    _sku("Reserved V5e TPU in Las Vegas in Calendar Mode", "OnDemand", 0.84),
    _sku("Commitment v1: TpuV5e running in Las Vegas for 1 Year", "Commit1Yr", 0.84),
    _sku("TpuV5e running in Tokyo", "OnDemand", 1.5410, region="asia-northeast1"),
]
...
class TestCostFromLivePricing(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_each_model_selects_its_own_sku(self):
        expected = {"on-demand": 1.20, "flex-start": 0.60, "spot": 0.5779}
        for model, rate in expected.items():
            rate_found, unit, desc = await _lookup_tpu_rate("v5e", model, "us-west4")
    ...
    async def test_reserved_and_committed_skus_are_not_mistaken_for_on_demand(self):
        """All three share a region and two share the OnDemand usage type."""
        _, _, desc = await _lookup_tpu_rate("v5e", "on-demand", "us-west4")
        self.assertEqual(desc, "TpuV5e running in Las Vegas")

    async def test_region_is_respected(self):
        rate, _, _ = await _lookup_tpu_rate("v5e", "on-demand", "asia-northeast1")
        assert rate is not None
        self.assertAlmostEqual(rate, 1.5410, places=4)

    async def test_missing_sku_reports_instead_of_guessing(self):
        rate, _, detail = await _lookup_tpu_rate("v5e", "spot", "europe-west4")
        self.assertIsNone(rate)
        result = await estimate_deployment_cost(tpu_type="v5e", provisioning_model="spot", region="europe-west4")
    ...
    async def test_cost_multiplies_rate_by_chips_and_hours(self):
        result = await estimate_deployment_cost(
            hours=2.0, tpu_type="v5e", topology="2x4", provisioning_model="on-demand", region="us-west4"
        )
        self.assertIn("$19.20", result)  # 1.20 * 8 chips * 2h
        self.assertIn("TpuV5e running in Las Vegas", result)

    async def test_ranking_matches_published_rates(self):
        """Spot really does land just under flex-start here — the old table had this wrong."""
        rates = {}
        for model in PROVISIONING_MODELS:
            rate, _, _ = await _lookup_tpu_rate("v5e", model, "us-west4")
            assert rate is not None, f"no SKU matched for {model}"
            rates[model] = rate
        self.assertLess(rates["spot"], rates["flex-start"])
        self.assertLess(rates["flex-start"], rates["on-demand"])
    ...
    def test_topology_parsing(self):
        ...
        self.assertIsNone(_parse_topology("v5p-8"))
...
_MT = "https://www.googleapis.com/compute/v1/projects/p/zones/us-east5-a/machineTypes/ct5p-hightpu-1t-tpu"


def _node(name, ip="10.0.0.1", state="RUNNING", external=True):
    """A `compute instances list` entry as gcloud prints it."""
    nic = {"networkIP": ip}
    if external:
        nic["accessConfigs"] = [{"natIP": ip}]
    return {
        "name": name,
        "status": state,
        "machineType": _MT,
...
class TestNodeDiscovery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_finds_standalone_spot_vm_with_no_queued_resource(self):
        ...
        with (
            patch("server.run_command", new=AsyncMock(side_effect=self._router([_node("legacy-devops-agent")]))),
            patch("server._probe_vllm", new=AsyncMock(return_value="google/gemma-4-E2B-it")),
        ):
            node = await _discover_vllm_node()
        assert node is not None
        self.assertEqual(node.name, "legacy-devops-agent")
    ...
    async def test_node_with_no_ip_is_skipped(self):
        nodes = [_node("pending-node", state="PROVISIONING")]
    ...
    async def test_resolve_node_id_falls_back_to_a_listed_instance(self):
        """`instances describe` missed it, but an instance with the name is right there."""
    ...
    async def test_resolve_node_id_prefers_a_direct_instance_hit(self):
        """An instance answering to the exact name short-circuits the list and the probe."""
        ...
        with patch("server.run_command", new=AsyncMock(side_effect=_run)):
            self.assertEqual(await _resolve_node_id(RESOURCE_ID), RESOURCE_ID)
    ...
    async def test_resolve_node_id_last_resort_is_the_serving_node(self):
        ...
        with (
            ...
            self.assertEqual(await _resolve_node_id(RESOURCE_ID), "legacy-devops-agent")

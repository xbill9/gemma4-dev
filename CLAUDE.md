...
Serving rigs: `tpu-jax-v5e1-2b`, `tpu-jax-v6e1-2b`, `tpu-pytorch-v5e1-12b`, `tpu-pytorch-v5e1-2b`, `tpu-pytorch-v6e1-2b`, `tpu-pytorch-inf2-2b` (AWS Inferentia2, not GCP), `tpu-vllm-v5e1-2b` (the live-demo rig), `tpu-vllm-v5p1-2b`, `tpu-vllm-v6e1-2b`, `tpu-vllm-v6e8-2b`, and the Compute Engine rigs `gce-vllm-v5e1-2b`, `gce-vllm-v5p1-2b`, `gce-vllm-v6e1-2b`, `gce-vllm-v6e8-2b`, `gce-vllm-v6e8-31b`, and the AMD rig `gpu-vllm-mi300x-2b`.

**`gpu-vllm-mi300x-2b` is new as of 2026-09-16** — the **first AMD rig here, and the first whose control plane is neither Google Cloud nor EC2**: it is one Instinct MI300X on a DigitalOcean GPU droplet reached through AMD Developer Cloud. Every `gcloud` reflex in the siblings is wrong inside it, and its closest relative is a different repo (`~/amd-gputools`). It serves `google/gemma-4-E2B-it` and has verified text, thinking, tool calling and vision against the live endpoint; it carries **one measured sweep** as of 2026-09-16 (`2026-09-16-vllm-sweep-mi300x`, 12 cells, 3 repeats, worst-cell cv 8.4%), whose headline is that the KV pool is never the constraint — 5.8% occupancy at the heaviest cell — so long-context throughput flattens on prefill rather than memory, inverting the TPU rigs' sizing rule. **Benchmarking it measures the prefix cache unless every cell gets a unique prompt seed** (worth 2.25x at 8192 context); the contaminated run is kept as `...-seedcollision`. Three things about it do not generalize: **the newest vendor image cannot load Gemma 4 at all** (`rocm/vllm` at vLLM 0.27.1.dev5 lacks `Gemma4ModelArchConfigConvertor` and raises `AmbiguousGlobalPerLayerAttributeError` on `head_dim`, because Gemma 4's sliding layers are 256-wide and its full-attention layers 512), so **newer is not safer and `check_image` runs before `VLLM_IMAGE` changes**; the official image's `ENTRYPOINT` is already `["vllm","serve"]` while AMD's images need the subcommand spelled out; and **audio is unreachable on every ROCm image** because none ships the `vllm[audio]` extras, so `audio: 0` is a fact rather than a preference. It is **not an A/B twin** of anything — nothing else here serves this checkpoint on AMD — so a number from it must never be differenced against a TPU or EC2 rig and read as a hardware result.

**`gce-vllm-v6e8-31b` is new as of 2026-08-25** — forked from `gce-vllm-v6e8-2b` and retargeted from E2B to `google/gemma-4-31B-it`. It is **the only serving rig here that does not serve a 2B checkpoint**, which makes it the one place where E2B reasoning is actively wrong rather than merely unverified: 960 KiB/token of KV against E2B's 18, TP=8 a floor rather than a choice (57.7 GiB of weights need ≥4 chips), and a 62 GB checkpoint pull that moved the boot-readiness budget from 20 minutes to 90. It has provisioned nothing and measured nothing; every capacity figure in it is labelled arithmetic. **It is not an A/B twin** — nothing here serves 31B through the Cloud TPU API, so a number from it must never be differenced against a 2B rig and read as a control-plane result.

**`tpu-vllm-v5p1-2b` is new as of 2026-08-10** — forked from `tpu-vllm-v5e1-2b`, repointed at TPU v5p, and then migrated off the Cloud TPU API onto **Compute Engine**. It was the first rig here off the deprecated control plane; it is no longer the only one — `gce-vllm-v5e1-2b`, `gce-vllm-v5p1-2b`, `gce-vllm-v6e1-2b` and `tpu-jax-v6e1-2b` also provision through Compute Engine. (That last name contradicts slot 1 of the naming scheme; `NAMING.md` records why it was kept and that it is not a precedent.) It is untracked so far, has provisioned nothing, and carries **no measurements of its own**: the v5e-1/v6e-1 artifacts that came with the fork were deleted rather than left to be counted against it by `benchmarks/rollup.py`. Three things about it do not generalize to the siblings, all written up in `@HARDWARE.md` — it provisions with `gcloud compute instances` (machine type `ct5p-hightpu-1t-tpu`, image family `ubuntu-accel-2204-amd64-tpu-v5e-v5p-v6e`) rather than queued resources; it runs **one** v5p chip at `TENSOR_PARALLEL_SIZE=1`, a size the TPU API cannot sell (its v5p floor is `v5p-8`, four chips); and its quota lives on `compute.googleapis.com`, not `tpu.googleapis.com`. **This project can reach v5p in only three zones** (`v5p-8` is published in ≥9, but quota and hardware overlap in three), of which only `us-east5-a` offers flex-start.
...
Four facts-not-code files at the root, each scoped to one axis that does **not** vary by rig, plus one that is **method rather than fact**. Read the relevant one before deriving its numbers yourself, and correct it there rather than restating it in a rig:
...
- **`@RIG-ANALYSIS.md`** — **method, not facts.** The order to consult the four above in, the drivers that decide whether a rig can work at all (compute-dtype match, largest *contiguous* block, transients, fused-kernel availability, provisioning), and a worksheet to fill in **before** provisioning. Read it before planning a new rig or a new model size — the arithmetic steps are free and the ones that cost a capacity cycle come last.

...
def main() -> int:
    ...
    ap.add_argument("--rig", default="gpu-pytorch-g5g-2b")
    ...
    engine = args.engine or {"jax": "jax"}.get(health.get("backend"), "pytorch-transformers")
    version = args.engine_version or "torch 2.12.0+cu132"
    max_len = args.max_model_len or health.get("seq") or 0
    weights_gib = args.weights_gib or round(
        (health.get("weight_bytes") or prom(metrics, "tpu_jax_weight_bytes")) / 2**30, 3)
    ...
    report = {
        ...
        "run": {
            ...
            "date": run_id[:10],
            "operator": "xbill",
            "source": (f"gpu-pytorch-g5g-2b/sweep.py + make_report.py; "
        ...
        "throughput": {
            "workload": {
                "tool": f"gpu-pytorch-g5g-2b/sweep.py (decode-source "

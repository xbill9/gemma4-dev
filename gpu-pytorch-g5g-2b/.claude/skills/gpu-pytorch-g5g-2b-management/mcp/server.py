...
The JAX path sidesteps all four:
  ...
  * jaxlib and the jax-cuda plugin/pjrt pair publish aarch64 wheels, and every
    ...
    (Named without a CUDA major on purpose: TORCH_PIP_SPEC has already moved 12→13,
...
SERVICE_NAME = os.getenv("SERVICE_NAME", "torch-g5g")
...
TORCH_PYTHON_VERSION = os.getenv("TORCH_PYTHON_VERSION", "3.12")
...
| Runtime | `{TORCH_PIP_SPEC}` on Python {TORCH_PYTHON_VERSION} |

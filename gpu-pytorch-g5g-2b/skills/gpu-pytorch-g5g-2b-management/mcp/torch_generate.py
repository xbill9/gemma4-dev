...
"""One-shot generate against the local checkpoint, for smoke-testing the box.
    ...
    python3 torch_generate.py --prompt "Explain Graviton in one sentence."
...
def main() -> None:
    ...
    p.add_argument("--prompt", default="Explain Graviton in one sentence.")
    ...
    if not torch.cuda.is_available():
        raise SystemExit(
            "no CUDA device. On this rig that usually means the AMI is an ARM64 "
            "DLAMI built for Graviton CPU inference -- it boots fine and has no "
            "GPU. Check `verify_gpu_arch` before anything else."
        )

#!/usr/bin/env python3
"""Refresh generated MCP files in the bundled G4dn PyTorch skill.
...
def main() -> int:
    # The serving payload travels with the skill so an installed copy under
    # ~/.claude/skills can still run deploy_torch_server: server.py resolves the
    # payload next to itself first, and only then looks up at the rig root.
    names = (
        "server.py",
        "project-setup.sh",
        "requirements.txt",

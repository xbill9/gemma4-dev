...
"""OpenAI-compatible HTTP/SSE server for Gemma 4 on an NVIDIA T4, under PyTorch.
...
Run on the G4dn instance with the interpreter that holds the DLAMI's torch (the
bootstrap records it in /opt/torch-g4dn/PYTHON_BIN):
...
def resolve_compute_dtype(device: torch.device) -> torch.dtype:
    ...
    that most of decode disappears into conversion. Turing (SM 7.5, the T4 in
...
def load_hf_token() -> None:
    ...
    a G4dn, so that path could only ever have failed here, silently, and left
...
class Metrics:
    ...
    def __init__(self, model_id: str, engine: KVCacheDecodeEngine) -> None:
        self.model_id = model_id
        self.engine = engine
        self.rig = os.getenv("RIG_NAME", "gpu-pytorch-g4dn-2b")
...
def create_app(engine: KVCacheDecodeEngine):
    ...
    app = FastAPI(title="Gemma 4 on NVIDIA T4 via PyTorch", version="1.0.0")
    app.add_middleware(

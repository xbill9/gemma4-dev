...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --instance-type TYPE     any g4dn size: xlarge (default), 2xlarge, 4xlarge,"
      echo "                           8xlarge, 12xlarge, 16xlarge, metal"
      echo "                           (every size has >= 16 GiB of host RAM, so unlike"
      echo "                            the G5g sibling none is rejected)"
      ...
      echo "                           (default: the rig directory, gpu-pytorch-g4dn-2b)"
...
# The list must agree with _G4DN_SIZES in server.py, which is what actually
# enforces it at launch. No size is rejected here: the G5g sibling refuses its
# 8 GiB size, and this family has none -- g4dn starts at 16 GiB.
case "$INSTANCE_TYPE" in
  g4dn.xlarge|g4dn.2xlarge|g4dn.4xlarge|g4dn.8xlarge|g4dn.12xlarge|g4dn.16xlarge|g4dn.metal) ;;
  *) err "unsupported instance type: $INSTANCE_TYPE (g4dn sizes only)" ;;
esac

# 🚀 Deploying Gemma 4 E2B QAT on Cloud TPU v6e-1 (flex-start) using JAX
...
> HBM occupancy, KV sizes — was taken on a **v6e-1**, which is the chip this page now
> describes, so they apply directly. They were taken through the **Cloud TPU API**,
> before this rig migrated to Compute Engine; that changes provisioning and not serving
> (the KV cache allocation came out at the same integer on both control planes), so
> they remain valid.
...
The `tpu-jax-v6e1-2b` server defaults to this shape (`ACCELERATOR_TYPE=v6e-1`,

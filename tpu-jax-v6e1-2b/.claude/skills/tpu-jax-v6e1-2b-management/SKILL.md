---
name: tpu-jax-v6e1-2b-management
description: Manage Google Cloud TPU capacity on Compute Engine, bare JAX dev VMs, and Gemma 4 vLLM serving on TPU. Use when the user asks about provisioning, finding, probing, listing, or destroying TPU VMs / flex-start capacity, JAX on TPU, starting or debugging vLLM on TPU (v6e, v5p), TPU quotas and zones, TPU cost estimates, benchmarking TPU serving, or the TPU devops MCP agent. Triggers include "TPU", "flex-start", "v6e", "Trillium", "JAX on TPU", "vLLM on TPU", "TPU quota", "TPU stockout".
---

# TPU Management — Compute Engine path

Operate Google Cloud TPU infrastructure: acquire capacity, run bare JAX or Gemma 4
vLLM workloads, and tear down. Two ways to act:

1. **Preferred — MCP agent tools.** If the `tpu-jax-v6e1-2b` MCP server is
...
target project and writes the `tpu-jax-v6e1-2b` entry into the project's `.mcp.json`,
...
approve the server when prompted; `/mcp` should list `tpu-jax-v6e1-2b`.

Manual alternative:

```bash
claude mcp add tpu-jax-v6e1-2b \
  ...
  -- python .claude/skills/tpu-jax-v6e1-2b-management/mcp/server.py
```
...
3. **Wait for ready — `RUNNING` is not it.** See the section below.
4. **Work.** `verify_jax_tpu` for a JAX box; `manage_vllm_docker` for a serving one.
...
tpu-jax-v6e1-2b   europe-west4-a  ct6e-standard-1t   RUNNING
```
...
`workload="jax"` by default), `find_tpu_vm` (zone sweep, records failures),
`probe_zone_capacity` (spot probe — stockout vs quota, in seconds),
`get_zones_with_available_quota` (both metrics), `wait_for_jax_ready`,
`verify_jax_tpu`, `wait_for_vllm_ready`, `create_cpu_debug_vm`,
...
## JAX on TPU (bare dev VMs) — the default workload

For direct JAX work — kernels, benchmarks, custom inference — provision with
`workload="jax"` (the default):
...
```console
$ gcloud compute instances list --filter="name=tpu-jax-v6e1-2b"
NAME              ZONE            MACHINE_TYPE       STATUS
tpu-jax-v6e1-2b   europe-west4-a  ct6e-standard-1t   RUNNING

...
async def create_tpu_vm_instance(
    ...
    workload='jax' (the default) installs a bare jax[tpu] environment and asserts the TPU
    is visible: no docker, no HF token, much faster boot. workload='vllm' auto-starts
    Gemma 4 serving via the vLLM startup script instead.
...
async def get_tpu_vm_serial_log(
    ...
    Success markers: 'JAX-BOOTLOADER: TPU environment ready.' (workload='jax') or
    ...
    # Both workloads announce themselves, so scan for either bootloader's banner.
    for i in range(len(lines) - 1, -1, -1):
        if "Starting JAX TPU Bootloader" in lines[i] or "Starting vLLM Bootloader" in lines[i]:
            lines = lines[i:]
            break
...
async def get_deployment_command(
    ...
    try:
        ...
        return f"❌ {e}"
    script = "startup_script_jax_template.sh" if workload == "jax" else "startup_script_template.sh"
    lines = [
...
async def get_help() -> str:
    ...
    return (
        ...
        "the port tells you the difference — `wait_for_jax_ready` / `wait_for_vllm_ready` read them.\n"
        ...
        "### 🧰 Available MCP Tools\n\n"
        "#### 🐳 Capacity & Lifecycle\n"
        "- **`create_tpu_vm_instance`**: Creates a TPU VM via Compute Engine; workload='jax' (default) "
        "installs a bare jax[tpu] dev environment, workload='vllm' auto-starts serving.\n"

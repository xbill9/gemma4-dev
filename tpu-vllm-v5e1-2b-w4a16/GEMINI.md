...
*   `MCP_SERVER_NAME`: Name this server advertises, and the key it must be registered under — it prefixes
    every tool as `mcp__<name>__find_tpu` (defaults to the rig directory name, `tpu-vllm-v5e1-2b-w4a16`).
...
model_list:
  - model_name: "gemma4-tpu"
    litellm_params:
      model: "openai/google/gemma-4-E2B-it-qat-w4a16-ct" # Tell LiteLLM it's an OpenAI-style endpoint
...
export GEMINI_MODEL="google/gemma-4-E2B-it-qat-w4a16-ct"

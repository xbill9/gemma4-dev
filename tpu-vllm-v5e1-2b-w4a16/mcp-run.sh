...
# after the rig directory (RIG_NAME in server.py), so it reports as tpu-vllm-v5e1-2b-w4a16.
# Register it in the client under that same name — the client's key is what prefixes the
# tools (mcp__tpu-vllm-v5e1-2b-w4a16__find_tpu), and sibling rigs all answering to one shared

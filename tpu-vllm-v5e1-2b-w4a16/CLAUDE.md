...
(`google/gemma-4-E2B-it-qat-w4a16-ct`) with vLLM on a Google Cloud TPU v5e-1 Flex-start Queued Resource. Its tools shell
...
resolve to `tpu-vllm-v5e1-2b-w4a16`. `RIG_NAME` also supplies the default `MCP_SERVER_NAME`, which names the
...
prefixes every tool: `mcp__tpu-vllm-v5e1-2b-w4a16__find_tpu`. All six rigs used to register as `tpu-devops`, so a
...
`benchmark` / `query` all `describe` a tpu-vm named `$(SERVICE_NAME)` = `tpu-vllm-v5e1-2b-w4a16`, while the MCP
tools manage a Queued Resource of the same name whose *node* is `tpu-vllm-v5e1-2b-w4a16-node`. Those are two

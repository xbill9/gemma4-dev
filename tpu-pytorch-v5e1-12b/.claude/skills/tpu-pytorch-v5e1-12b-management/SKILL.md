---
name: tpu-pytorch-v5e1-12b-management
...
1. **Preferred — MCP agent tools.** If the `tpu-pytorch-v5e1-12b` MCP server is
...
target project and writes the `tpu-pytorch-v5e1-12b` entry into the project's `.mcp.json`,
...
approve the server when prompted; `/mcp` should list `tpu-pytorch-v5e1-12b`.

Manual alternative:

```bash
claude mcp add tpu-pytorch-v5e1-12b \
  ...
  -- python .claude/skills/tpu-pytorch-v5e1-12b-management/mcp/server.py
```

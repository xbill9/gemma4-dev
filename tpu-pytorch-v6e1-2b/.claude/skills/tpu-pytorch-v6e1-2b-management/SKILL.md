---
name: tpu-pytorch-v6e1-2b-management
...
1. **Preferred — MCP agent tools.** If the `tpu-pytorch-v6e1-2b` MCP server is
...
target project and writes the `tpu-pytorch-v6e1-2b` entry into the project's `.mcp.json`,
...
approve the server when prompted; `/mcp` should list `tpu-pytorch-v6e1-2b`.

Manual alternative:

```bash
claude mcp add tpu-pytorch-v6e1-2b \
  ...
  -- python .claude/skills/tpu-pytorch-v6e1-2b-management/mcp/server.py
```

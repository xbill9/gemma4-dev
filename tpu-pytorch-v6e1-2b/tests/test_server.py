"""Smoke and regression tests for the tpu-pytorch-v6e1-2b MCP server and repo invariants.
...
class StartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/tpu-pytorch-v6e1-2b-management/mcp/startup_script_template.sh"
        ).read_text()
...
class PyTorchStartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/tpu-pytorch-v6e1-2b-management/mcp/startup_script_pytorch_template.sh"
        ).read_text()
        return template.format(
            project_id="test-project",
            zone="europe-west4-a",
    ...
    def test_smoke_test_follows_tpu_rules(self):
        """The embedded smoke test must use plain torch (never importing the backend
        or PyTorch/XLA) and compile with backend="tpu"."""
        rendered = self.render()
        smoke = rendered.split("<< 'PYEOF'")[1].split("PYEOF")[0]
        # Plain torch only: the smoke test must import nothing but torch — importing
        # the backend package directly (or PyTorch/XLA) is the failure this guards.
        imports = {
            line.split()[1].split(".")[0]
            for line in smoke.splitlines()
            if line.startswith(("import ", "from "))
        }
        self.assertEqual(imports, {"torch"}, f"unexpected imports in smoke test: {imports}")
...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_skill_snapshots_in_sync_with_sources(self):
        ...
        for src, snap in (
            ("server.py", ".claude/skills/tpu-pytorch-v6e1-2b-management/mcp/server.py"),
            ("project-setup.sh", ".claude/skills/tpu-pytorch-v6e1-2b-management/mcp/project-setup.sh"),
            ("requirements.txt", ".claude/skills/tpu-pytorch-v6e1-2b-management/mcp/requirements.txt"),
        ):
            self.assertTrue(
    ...
    def test_plugin_copy_matches_skill(self):
        for rel in (
            ...
            self.assertTrue(
                filecmp.cmp(
                    ROOT / ".claude/skills/tpu-pytorch-v6e1-2b-management" / rel,
                    ROOT / "skills/tpu-pytorch-v6e1-2b-management" / rel,
                    shallow=False,
                ),
                f"skills/tpu-pytorch-v6e1-2b-management/{rel} is stale — run `make skill`",
            )

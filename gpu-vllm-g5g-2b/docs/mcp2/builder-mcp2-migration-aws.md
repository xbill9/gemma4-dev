...
*Subtitle: What basic MCP Python SDK 2.x support takes for an AWS EC2 MCP server built on boto3: one rename in server.py, snake_case in the tests, a floor in requirements.txt, and why neither the wire format nor the boto3 calls change.*
...
| `httpx` replaced by `httpx2` | `No module named 'httpx'` |  exposed, already declared |
...
-  Upgrading a shared Python to mcp 2.2.0 put `strands-agents` 1.55.0 outside its `mcp<2.2` bound and stopped 37 other FastMCP servers until they are migrated

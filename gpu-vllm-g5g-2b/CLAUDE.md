...
list.** `make skill` regenerates the snapshots under `.claude/skills/` and `skills/` — those
are generated copies, so edit the sources and re-run, never hand-edit them.
...
provisioned nothing. `benchmarks/` carries only the synced schema and README; the L4 4B

# 🤖 Gemini Workspace Context: TPU Management Skill & tpu-vllm-v6e8-2b MCP Agent
...
This repository packages a Claude Code skill (`tpu-vllm-v6e8-2b-management`) and a **Model Context Protocol (MCP) server** (`tpu-vllm-v6e8-2b`) that together act as an AI DevOps/SRE agent for Google Cloud TPUs. Two main purposes:
...
- **Reference guide:** `.claude/skills/tpu-vllm-v6e8-2b-management/references/tpu-guide.md` — TPU getting started guide: zones, quotas, troubleshooting
...
make skill-package # ...and rebuild dist/tpu-vllm-v6e8-2b-management-skill.zip
```

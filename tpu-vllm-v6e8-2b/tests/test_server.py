"""Smoke and regression tests for the tpu-vllm-v6e8-2b MCP server and repo invariants.
...
class StartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/tpu-vllm-v6e8-2b-management/mcp/startup_script_template.sh"
        ).read_text()
...
class PyTorchStartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/tpu-vllm-v6e8-2b-management/mcp/startup_script_pytorch_template.sh"
        ).read_text()
        return template.format(
            project_id="test-project",
            zone="europe-west4-a",
...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_skill_snapshots_in_sync_with_sources(self):
        ...
        for src, snap in (
            ("server.py", ".claude/skills/tpu-vllm-v6e8-2b-management/mcp/server.py"),
            ("project-setup.sh", ".claude/skills/tpu-vllm-v6e8-2b-management/mcp/project-setup.sh"),
            ("requirements.txt", ".claude/skills/tpu-vllm-v6e8-2b-management/mcp/requirements.txt"),
        ):
            self.assertTrue(
    ...
    def test_plugin_copy_matches_skill(self):
        for rel in (
            ...
            self.assertTrue(
                filecmp.cmp(
                    ROOT / ".claude/skills/tpu-vllm-v6e8-2b-management" / rel,
                    ROOT / "skills/tpu-vllm-v6e8-2b-management" / rel,
                    shallow=False,
                ),
                f"skills/tpu-vllm-v6e8-2b-management/{rel} is stale — run `make skill`",
            )

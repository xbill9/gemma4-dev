---
name: tpu-vllm-v6e8-2b-management
...
1. **Preferred — MCP agent tools.** If the `tpu-vllm-v6e8-2b` MCP server is
...
target project and writes the `tpu-vllm-v6e8-2b` entry into the project's `.mcp.json`,
...
approve the server when prompted; `/mcp` should list `tpu-vllm-v6e8-2b`.

Manual alternative:

```bash
claude mcp add tpu-vllm-v6e8-2b \
  ...
  -- python .claude/skills/tpu-vllm-v6e8-2b-management/mcp/server.py
```

...
# after the rig directory (RIG_NAME in server.py), so it reports as gce-vllm-v6e1-2b.
# Register it in the client under that same name — the client's key is what prefixes the
# tools (mcp__gce-vllm-v6e1-2b__find_tpu), and sibling rigs all answering to one shared

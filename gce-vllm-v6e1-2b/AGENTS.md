...
v6e-1 (Trillium) Flex-start Queued Resource. The main application is a single-file MCP server (`MCPServer`) in `server.py`. Its
...
- A v6e-1 is a single chip, so the correct default tensor parallel size is `1`. Older documentation and some
  Makefile examples incorrectly show `4`.
- gcloud spells v6e `v6e`, with none of the `v5litepod` renaming v5e has. The accelerator type is `v6e-1`,
  the Flex-start runtime version is `v2-alpha-tpuv6e`, and `--type=v6e --topology=1x1` is the tpu-vm form.
  The quota ids are `TPUV6EPerProjectPerZoneForTPUAPI` and `TPUV6EPreemptiblePerProjectPerZoneForTPUAPI`
  (no `Litepod`). Read them from `tpu.env` / `server.py` rather than from the directory name — `v6e1`, the
  directory spelling, is still not a valid gcloud argument.
- The default zone is `us-east5-b`. us-west4, which is where the v5e rig this was forked from provisions,
  has no v6e hardware, quota, or SKU at all.

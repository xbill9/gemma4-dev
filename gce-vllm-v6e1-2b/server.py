...
async def _list_queued_resources_json(zone: str) -> Optional[list]:
    ...
    sibling tpu-vllm-v6e1-2b provisions QRs into this same project and zone, and a chip it
    holds is a chip this rig cannot get. Nothing here creates or deletes one.
    """
    cmd = [
        "gcloud",
        "alpha",
...
async def get_vllm_deployment_config(service_name: str = INSTANCE_NAME, model_name: str = MODEL_NAME) -> str:
    """Generates the gcloud command for a single-chip v6e vLLM deployment on Compute Engine.

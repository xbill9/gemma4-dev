"""Smoke and regression tests for the gce-jaxrust-v6e1-2b MCP server and repo invariants.
...
class StartupTemplateTests(unittest.TestCase):
    def render(self):
        template = (
            ROOT / ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/startup_script_template.sh"
        ).read_text()
...
class JaxStartupTemplateTests(unittest.TestCase):
    ...
    def render(self):
        template = (
            ROOT / ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/startup_script_jax_template.sh"
        ).read_text()
        return template.format(
            project_id="test-project",
            zone="europe-west4-a",
...
class JaxrustStartupTemplateTests(unittest.TestCase):
    """The Rust/XLA startup script carries the same failure discipline as the JAX one,
    plus two checks that are specific to it.

    `protoc` and `clang` are not optional: pjrt-sys runs prost-build and bindgen in its
    build script, and their absence surfaces as a build failure minutes into a cargo
    build rather than as a missing package. And a file named libtpu.so is not a PJRT
    plugin — the thing that makes it one is the GetPjrtApi symbol, so the script asserts
    on the symbol rather than on the path existing.
    """

    def render(self):
        return server._get_jaxrust_startup_script("europe-west4-a")

    def test_renders_without_leftover_placeholders(self):
        rendered = self.render()
        for placeholder in ("{project_id}", "{zone}", "{rust_toolchain}", "{libtpu_spec}"):
            self.assertNotIn(placeholder, rendered)

    def test_rendered_script_passes_bash_syntax_check(self):
        proc = subprocess.run(
            ["bash", "-n", "/dev/stdin"], input=self.render(), text=True, capture_output=True
        )
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_fails_loudly(self):
        rendered = self.render()
        self.assertRegex(rendered, r"set -eu?x?", "script must exit on error")
        self.assertIn("trap ", rendered, "script must install an ERR trap")
        self.assertIn("JAXRUST-BOOTLOADER: FAILED", rendered)
        self.assertIn("JAXRUST-BOOTLOADER: TPU environment ready.", rendered)

    def test_err_trap_is_installed_before_tracing(self):
        """With `set -x` on first, the trap definition is itself echoed — and that
        trace line contains the FAILED marker, so a scanner reports failure on a
        healthy boot."""
        rendered = self.render()
        self.assertLess(
            rendered.index("trap "),
            rendered.index("set -x"),
            "the ERR trap must be installed before tracing is enabled",
        )
    ...
    def test_installs_libtpu_without_jax(self):
        ...
        self.assertNotIn("jax[tpu]", rendered)
...
class TemplateResolutionTests(unittest.TestCase):
    ...
    def test_jaxrust_template_renders_from_repo_root(self):
        script = server._get_jaxrust_startup_script("europe-west4-a")
        self.assertIn("JAXRUST-BOOTLOADER", script)
        proc = subprocess.run(
            ["bash", "-n", "/dev/stdin"], input=script, text=True, capture_output=True
        )
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_deployed_copy_is_a_sibling_of_its_templates(self):
        ...
        mcp_dir = ROOT / ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp"
        for name in (
            "server.py",
            "startup_script_template.sh",
            "startup_script_jax_template.sh",
            "startup_script_jaxrust_template.sh",
        ):
...
class TemplateBraceHygieneTests(unittest.TestCase):
    ...
    TEMPLATES = {
        ...
        "startup_script_jaxrust_template.sh": {
            "project_id", "zone", "rust_toolchain", "libtpu_spec",
        },
    ...
    def test_only_known_placeholders_appear(self):
        ...
        for name, allowed in self.TEMPLATES.items():
            text = (ROOT / ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp" / name).read_text()
    ...
    def test_every_template_renders(self):
        ...
        for render in (
            ...
            lambda: server._get_jaxrust_startup_script("europe-west4-a"),
...
class OffTheCloudTpuApiTests(unittest.TestCase):
    ...
    SOURCES = (
        "server.py",
        "Makefile",
        ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/startup_script_template.sh",
        ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/startup_script_jax_template.sh",
        ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/startup_script_cpu_template.sh",
    )
...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_skill_snapshots_in_sync_with_sources(self):
        ...
        for src, snap in (
            ("server.py", ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/server.py"),
            ("project-setup.sh", ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/project-setup.sh"),
            ("requirements.txt", ".claude/skills/gce-jaxrust-v6e1-2b-management/mcp/requirements.txt"),
        ):
            self.assertTrue(
    ...
    def test_plugin_copy_matches_skill(self):
        for rel in (
            ...
            self.assertTrue(
                filecmp.cmp(
                    ROOT / ".claude/skills/gce-jaxrust-v6e1-2b-management" / rel,
                    ROOT / "skills/gce-jaxrust-v6e1-2b-management" / rel,
                    shallow=False,
                ),
                f"skills/gce-jaxrust-v6e1-2b-management/{rel} is stale — run `make skill`",
            )

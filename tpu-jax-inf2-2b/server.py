...
# (Inferentia2 chips, host RAM GiB). Read from `describe_instance_types` on
# 2026-08-28 -- note Inferentia reports under `InferenceAcceleratorInfo`, NOT
# `AcceleratorInfo`, which comes back empty and reads as "no accelerator".
#
# THE TRAP, and it is the whole reason this table carries host RAM:
# **inf2.xlarge and inf2.8xlarge have the SAME accelerator** -- 1 chip, 2 cores,
# 32,768 MiB total, 16 GiB per core on both. 8xlarge is 2.6x the price
# ($1.9679/hr on-demand against $0.7582; $0.5552 spot against $0.1417) and buys
# ZERO extra compute or accelerator memory. It buys host RAM: 16 GiB -> 128 GiB.
#
# The API's 16.0 GiB per core independently confirms HARDWARE.md's measured
# `hbm_limit_bytes` = 17,179,869,184. Accelerator memory is PER CORE, not a
# pool, so a single-device engine has 16 GiB on every size in this table --
# including the 12-chip 48xlarge.
_INF2_SIZES = {
    "inf2.xlarge": (1, 16),
    "inf2.8xlarge": (1, 128),
    "inf2.24xlarge": (6, 384),
    "inf2.48xlarge": (12, 768),
}


def _neuron_devices(instance_type: str) -> int:
    return _INF2_SIZES.get(instance_type, (0, 0))[0]


def _host_memory_gb(instance_type: str) -> int:
    return _INF2_SIZES.get(instance_type, (0, 0))[1]


def _needs_swap(instance_type: str) -> bool:
    """True when host RAM is too small for the load peak.

    INCLUSIVE at 16 GiB, the same boundary the `gpu-jax` line settled on: the
    one-time load peaks ~14.5 GB of host RAM against inf2.xlarge's exactly 16
    GiB, and `< 16` would give the one size that needs a swapfile none. This is
    the only thing a bigger inf2 actually fixes.
    """
    return 0 < _host_memory_gb(instance_type) <= 16


def _oversized_for_single_device(instance_type: str) -> bool:
    """More than one chip, which a single-device engine cannot use.

    Not an error -- you may have a reason -- but the caller should say so rather
    than pay for 12 chips and drive one. The `gpu-jax` line carries the same
    warning for g5g.16xlarge/metal.
    """
    return _neuron_devices(instance_type) > 1

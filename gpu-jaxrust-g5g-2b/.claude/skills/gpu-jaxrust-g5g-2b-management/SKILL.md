---
name: gpu-jaxrust-g5g-2b-management
description: Manage AWS EC2 G5g capacity (Graviton2 + NVIDIA T4G) and Gemma 4 E2B serving with a Rust-driven XLA engine. Use when the user asks about provisioning, launching, listing, or terminating G5g instances, installing or debugging JAX/XLA on T4G / Turing / aarch64, G-family quotas, or the gpu-jaxrust-g5g-2b devops MCP agent. Triggers include "G5g", "T4G", "Graviton", "Turing", "SM 7.5", "arm64 GPU", "jaxrust", "Rust JAX", "xla-rs", "JAX on GPU".
---

# gpu-jaxrust-g5g-2b management

Provision and operate **EC2 G5g** (AWS Graviton2 host + NVIDIA **T4G** GPU) serving
`google/gemma-4-E2B-it` through the `gpu-jaxrust-g5g-2b` MCP server.

This is the **Rust** sibling of `gpu-jax-g5g-2b` and `gpu-vllm-g5g-2b`. Same hardware, same
checkpoint, different runtime — and the runtime is the whole point.

**The Rust engine is not written yet.** This rig currently runs the Python JAX engine inherited
from the fork, and has measured nothing. `docs/rust-jax-runtime-survey.md` has the verdict:
`xla-rs` is the only live candidate, the aarch64+CUDA-13 binary exists, and what remains is a
3,764-line port rather than a dependency swap. Do not quote `gpu-jax-g5g-2b`'s 13.1 tok/s here.
...
- Instance discovery is scoped to `ManagedBy=gpu-jaxrust-g5g-2b`. Never operate outside that tag.

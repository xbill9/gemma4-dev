...
Serves **`google/gemma-4-E2B-it`** on **AWS EC2 G5g** — Graviton2 (aarch64) host, **NVIDIA
T4G** GPU (Turing, SM 7.5, **15360 MiB measured**) — with the engine intended to be **driven
from Rust rather than CPython**.

**This rig has served nothing.** Forked from `gpu-jax-g5g-2b` 2026-08-28; the Rust engine is
not written and the Python JAX engine inherited from the fork is the current fallback and
parity oracle. `benchmarks/` is deliberately empty. Read `docs/rust-jax-runtime-survey.md`
before touching the engine, and never quote `gpu-jax-g5g-2b`'s 13.1 tok/s as this rig's.
...
- Scope instance discovery to `ManagedBy=gpu-jaxrust-g5g-2b`.

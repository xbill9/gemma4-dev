# CLAUDE.md — `gpu-jaxrust-g5g-2b`

Serving rig: **`google/gemma-4-E2B-it`** on **AWS EC2 G5g** — a Graviton2 (aarch64) host with
an **NVIDIA T4G** GPU (Turing, SM 7.5, **15360 MiB** measured, not the nominal 16 GB) — with the
engine intended to be **driven from Rust rather than CPython**.

Forked from `gpu-jax-g5g-2b` on **2026-08-28**. Same chip, same checkpoint, same MCP control
plane; the only intended difference is slot 2. `NAMING.md` defines `jaxrust` as "JAX-family graph
execution driven from Rust — the forward pass is defined in Rust and lowered to XLA/StableHLO, not
written in Python."
...
## This rig has served nothing, and its name is a claim about work not yet done

**Zero measurements. Do not quote a number as this rig's.** It was forked four hours before this
line was written and has provisioned nothing.

**The inherited benchmark artifacts were deleted rather than carried**, following the
`tpu-vllm-v5p1-2b` precedent and the monorepo rule that fork-travelled JSON is how rigs come to
claim hardware they never ran on. Nine runs and three reports measured on `gpu-jax-g5g-2b` are
gone from `benchmarks/`; they still exist in that rig, which is where they belong.

**`gpu-jax-g5g-2b` is the A/B control, and it is still the one that works.** It serves **13.1
tok/s** single-stream. That number is *its* result. A `jaxrust` figure is only meaningful
differenced against it, and only once this rig produces one.

## Python is still here, as the parity oracle

**The Rust engine is written (see below) but has never run.** `jax_engine.py`,
`jax_openai_server.py` and `ports/gemma4/` are the Python JAX implementation inherited from the
fork, they work, and they stay. Two reasons, and the second is the load-bearing one:

1. **They are the only implementation here whose numbers have been checked**, so a disagreement
   between the two engines is evidence about the Rust engine, not about the model.
2. **They are what serves if the Rust path does not.** Until a G5g run says otherwise, this rig's
   slot 2 is a claim about intent, and the directory name runs ahead of the evidence. Recorded
   here rather than hidden.

`gemma4-engine` is the target and `--device cpu` is how parity against the Python port gets done
off the chip.

**Read `docs/rust-jax-runtime-survey.md` before changing the engine.** Surveyed 2026-08-28; the
short version, because it kills the two obvious approaches:

- **`rlx` is the candidate** — "JAX-shaped IR, autodiff, vmap" over CPU / NVIDIA / TPU backends,
  v0.2.14 released 2026-08-12, and it ships **model builders**, not just an op API. Crucially,
  **`gce-jaxrust-v6e1-2b` already builds against it** (`rlx`, `rlx-runtime`, `rlx-gemma` pinned
  `=0.2.11`). Read that rig before writing anything here — it is the same migration one chip over
  and it is further along.
- **`xla-rs` is the fallback** — raw bindings to the XLA C++ library, self-described
  "experimentation", but real. Its blocking question resolved **yes**:
  `xla_extension-0.10.0-aarch64-linux-gnu-cuda13.tar.gz` is published, `cuda13` matches this rig's
  stack, and glibc 2.43 clears the 2.31 floor — so no from-source XLA build.
- **`nox` is abandoned** (last published 2024-08-23) and **`jax-rs` is a dead end** — 13 stars,
  7 commits, **WebGPU only, no CUDA**, against a README claiming "100% feature parity". Believe
  the download count, not the README.

**The first draft of the survey was wrong and said `xla-rs` was the only live option.** It missed
`rlx` because searching for "Rust JAX" surfaces the crates *named* after JAX, not the one actually
shaped like it — which was already in use one directory over. **Check the sibling rigs before the
registry.**

Three things this rig must verify that the v6e-1 rig did not have to: whether `rlx` builds for
**aarch64**; whether its `cuda` feature works against Turing; and whether the Gemma builder assumes
**bf16**, which this chip does not have. Each is a build, not a launch.

Two traps the survey records and a fresh port will otherwise walk into:

- **Nothing would supply CUDA any more.** This rig installs without a toolkit because *pip*
  ships the CUDA libraries and the DLAMI supplies only the driver. **A Rust binary has no pip.**
  CUDA 13, cuDNN and NCCL must then come from apt, the DLAMI, or be vendored. Not a build, but
  the one place the "no toolkit" claim stops transferring — and it is **sharper for `rlx`**, whose
  CUDA backend is cuBLAS + cuDNN + **NVRTC** rather than PJRT. The compensation is that NVRTC
  compiles at runtime, so **SM 7.5 becomes a JIT target rather than a cubin-arch-table question**:
  the exact failure that breaks the vLLM sibling cannot happen that way.
- **The 2,030-line model file encodes findings that are invisible in its shape.** The
  padding-eviction invariant, the device-read dtype policy, opt-in `release_source`. A rewrite
  that reproduces the code's structure but not these reproduces the bugs — and the eviction bug's
  failure mode is a `status="success"` token loop, not a crash. Port bottom-up against a parity
  harness.

## The Rust engine exists, compiles, and has never run

`rust/` is a Cargo **workspace** with two crates. `make rust-test` (13 tests), `make rust-lint`
(fmt + clippy `-D warnings`), `make rust-check-cuda` (typechecks the CUDA path anywhere),
`make rust-build` (the real binary: `--no-default-features --features cuda,gemma`).

| Crate | What it is |
| --- | --- |
| `gemma4-geometry` | E2B geometry, the KV/padding invariants, dtype conversion. No GPU, no XLA. |
| `gemma4-engine` | The serving binary: `rlx` + `rlx-gemma` on `Device::Cuda`, axum, OpenAI-compatible. |

**It has never loaded a weight or generated a token.** Everything below is a build-time or
source-level fact; nothing here is measured. That distinction is the whole point of this section.

### `rlx` is the runtime, and finding it was the hard part

**`rlx` is "the Rust version of JAX"** — a JAX-shaped IR with autodiff and vmap over CPU / NVIDIA /
TPU backends — and `rlx-gemma` ships a Gemma 4 E2B implementation. That is why the port is an engine
and an HTTP surface rather than the 3,764-line model rewrite this rig first estimated.

**Two pieces of metadata lie in opposite directions, and both cost time:**

- **`jax-rs` oversells.** The top hit for "Rust JAX", README claiming "100% feature parity" and
  "production ready" — actually 13 stars, 7 commits, **WebGPU only, no CUDA**.
- **`rlx-gemma` undersells.** crates.io says "Gemma / **Gemma 2** causal LMs", which reads as a hard
  no for this rig. The source says `gemma_e2b.rs`, `gemma4_e2b_mm.rs`, `GemmaArch::Gemma4`,
  `global_head_dim`, `per_layer_inputs` (PLE), `qat_loader.rs`. **Read the source, not the
  registry** — in both directions.

`gce-jaxrust-v6e1-2b` was already building against `rlx` before this rig started, and searching the
registry never would have surfaced it. **Check the sibling rigs first.**

### What is verified, and what is not

**Verified by building:** `Device::Cuda` exists ("NVIDIA GPU via native CUDA (cuBLAS, cuDNN)");
`cargo check --features cuda,gemma` succeeds **on x86_64 with no driver and no toolkit**, because
`rlx-cuda` goes through `cudarc`/`libloading` and **dlopens CUDA at runtime**. So the CUDA path
needs no build-time toolkit — the whole engine was validated before any G5g was launched.

**NOT verified, in rough order of risk:**

- **aarch64.** Everything above was checked on x86_64. `rlx` documents ARMv7E-M for
  microcontrollers and says nothing about aarch64 for the main pipeline. Graviton2 + CUDA is the
  same rare axis that makes this whole rig hard.
- **bf16.** Turing has no bf16 datapath. If the Gemma builder assumes it, **it will not error — it
  will emulate through fp32**, which is exactly the 86.8% tax the Python rig pays today.
- **Runtime CUDA libraries.** The Python rig gets CUDA from *pip*. A Rust binary has no pip, so
  libcuda/cuBLAS/cuDNN must come from the DLAMI or apt. A runtime packaging question, not a build
  dependency — smaller than it looked, but not nothing.

### Versions: everything is latest except the rlx family, which cannot be

**Checked 2026-08-28.** Every direct dependency resolves to its newest published release — axum
0.8.9, clap 4.6.6, tokio 1.53.1, tokio-stream 0.1.19, half 2.7.1, tracing-subscriber 0.3.23,
anyhow 1.0.104, serde 1.0.229, serde_json 1.0.151 — and the Python requirements are deliberately
unpinned so pip takes latest.

**The 14 `rlx-*` crates sit at 0.2.11 against an available 0.2.14, and that is not our pin.**
`rlx-gemma` 0.2.11 hard-pins `rlx-runtime = "=0.2.11"`, and cargo cannot carry two
semver-compatible copies of one crate — so `rlx-runtime = "0.2"` still resolves to 0.2.11.
**0.2.11 is also the newest `rlx-gemma` in existence**: `rlx` and `rlx-runtime` released 0.2.14
and the model crate did not follow. Same split that left it GPL-3.0-only.

The manifests therefore carry **ranges, not pins**, so both move on their own the moment
`rlx-gemma` publishes again. Do not "fix" this by pinning `=0.2.11` — that would turn a temporary
upstream floor into a permanent one. `cargo update --dry-run --verbose` prints the held-back list
if you want to check it has not lifted.

### Licensing — read before shipping a binary

**`rlx-gemma` is GPL-3.0-only and stops at 0.2.11.** `rlx` itself relicensed to MIT/Apache-2.0 at
**0.2.14, but `rlx-gemma` never published 0.2.14**, so the permissive relicense does *not* reach the
model crate. Linking it makes the **binary** a GPLv3 combined work. This rig ships source and builds
on the instance, so nothing is distributed as a binary today — the moment one is, that is the
license it carries. The `gemma` cargo feature is the gate: without it the server still starts,
answers `/health`, reports the device, and refuses generation naming the flag.

### Decisions the engine encodes, each because it cost something here

- **One model thread, requests queued over a channel.** rlx's handles are `Rc`-based so the runner
  is `!Send`; a `Mutex` would not compile across an await and would be wrong anyway — one T4G runs
  one graph at a time, and `MAX_NUM_SEQS=1`.
- **`MAX_MODEL_LEN` defaults to 4096, not 8192** — measured, 4,105 serves and 5,120 is infeasible.
- **`max_memory_gb` defaults to 14.07**, the measured T4G budget.
- **Metric names match the Python rig exactly** (`tpu_jax_decode_tokens_per_second`, …) with a `rig`
  label. Both benchmark reports compare **by name**, so renaming the prefix would break continuity
  with every existing report.
- **`temperature`/`top_p` are accepted and ignored, and the response says so** in
  `system_fingerprint`. Sampling is fixed at build time because changing it recompiles the graph.
- **`stream: true` is honoured**, not silently downgraded to a single blob.
- **`max_tokens` clamping warns**, because the clamp changes the compiled shape.
- **The degenerate detector looks for a repeating CYCLE, not an identical token.** The eviction bug
  produced "a token repeated four times running", but the vLLM sibling's measured body was
  `': ok: ok: ok…'` — an alternating two-token cycle that an identical-adjacent test walks past.
  Both are the same failure. **Never health-check by testing for a non-empty response.**

**Everything below this line still applies**, because the chip, the checkpoint and the
provisioning path are unchanged by the choice of host language. Where the text says "JAX", read
it as the XLA-lowered execution model — the dtype policy, the memory arithmetic and the Turing
ceilings are properties of the device and the model, not of CPython.
...
- Scope instance discovery to `ManagedBy=gpu-jaxrust-g5g-2b`. **`MANAGED_BY = RIG_NAME`, so the tag
  is derived, not written down** — which is what keeps this rig and `gpu-jax-g5g-2b` from listing and
  terminating each other's instances. Never pin it to a literal. Unlike the inf2 rig, which keeps a
...
`gpu-jaxrust-g5g-2b`, which prefixes every tool as `mcp__gpu-jaxrust-g5g-2b__…`. All four were
renamed together on 2026-08-28 and agree. A mismatch makes `/mcp` and the tool prefix disagree about what this rig is.
...
`server.py` was right throughout (`RIG_NAME = "gpu-jaxrust-g5g-2b"`, asserted by
...
and it gained a `gpu-jax-g5g-2b` entry on 2026-08-18. **`gpu-jaxrust-g5g-2b` still needs one.**
...
**This rig has no measurements.** `benchmarks/runs/` and `benchmarks/reports/` are empty, and that
is correct rather than pending cleanup: the nine runs and three reports that came with the fork were
measured on `gpu-jax-g5g-2b`'s Python engine and were removed so `benchmarks/rollup.py` cannot count
them here. Same precedent as `tpu-vllm-v5p1-2b`.

When this rig does measure something, the run label must make the runtime legible — `<date>-<what>-g5g`
collides with the sibling's names, and the two rigs are only worth comparing if you can tell which
produced which. Prefer a `<what>` that carries the engine, e.g. `2026-09-01-first-serve-jaxrust-g5g`.

**The number to beat, and never to borrow: 13.1 tok/s** on `gpu-jax-g5g-2b`, `ple4+int8head`,
`g5g.2xlarge`. It is the A/B control for this rig and it belongs to the sibling. The same rule kills
three others:
...
`gpu-jax-g5g-2b` entries. The `jaxrust` fork repeated the exercise on 2026-08-28.

...
Serve **`google/gemma-4-E2B-it`** on **AWS EC2 G5g** — an AWS Graviton2 (64-bit Arm) host
paired with an **NVIDIA T4G Tensor Core** GPU — with the engine **driven from Rust rather
than CPython**.

> **Status: this rig has served nothing.** Forked from
> [`gpu-jax-g5g-2b`](../gpu-jax-g5g-2b/) on 2026-08-28 to move the engine off CPython onto
> **Rust-driven XLA**. The Rust engine is **written and compiles** (`rust/gemma4-engine`, on
> `rlx` + `rlx-gemma` with `Device::Cuda`) but **has never run** — no weights loaded, no token
> generated, aarch64 unverified. The inherited Python JAX engine remains the fallback and the
> parity oracle, and `benchmarks/` is deliberately empty. `gpu-jax-g5g-2b` remains the working rig and the A/B control at
> **13.1 tok/s**; that number belongs to it, not here.
> See [`docs/rust-jax-runtime-survey.md`](docs/rust-jax-runtime-survey.md) for what the
> migration actually costs.

No PyTorch, no torch_xla, no vLLM. The engine is this repo's own Gemma 4 port
(`ports/gemma4/`) driven by `jax_engine.py` behind an OpenAI-compatible FastAPI server
...
| Runtime | **target:** Rust `rlx` on CUDA (`rust/gemma4-engine`, builds, unproven) · **today:** Python JAX + XLA-GPU, OpenAI-compatible API on `:8000` |
...
Then, through the MCP tools (`mcp__gpu-jaxrust-g5g-2b__…`):
...
**Apache-2.0** ([`../LICENSE`](../LICENSE)) — except a binary built with the `gemma` cargo
feature, which links GPL-3.0-only `rlx-gemma` and is therefore a **GPLv3 combined work**.
Publishing source and serving the endpoint owe nothing; shipping a built artifact does. See
[`rust/README.md`](rust/README.md) and the licensing note in
[`rust/gemma4-engine/Cargo.toml`](rust/gemma4-engine/Cargo.toml).

...
"""Turn a sweep.json into a schema-valid serving report plus its REPORT.md.
    ...
    python3 make_report.py --run benchmarks/runs/2026-08-29-first-serve-g6 \
...
def main() -> int:
    ...
    ap.add_argument("--instance-type", default="g6.2xlarge")
    ap.add_argument("--vcpus", type=int, default=8)
    ap.add_argument("--host-ram-gb", type=int, default=32)
    args = ap.parse_args()
    ...
    report = {
        ...
        "run": {
            ...
            "date": run_id[:10],
            "operator": "xbill",
            "source": "sweep.py + make_report.py in gpu-pytorch-g6-2b",
            "notes": (
        ...
        # HARDWARE IS THIS RIG'S, and it is written out rather than derived
        # because a wrong constant here is the misattribution the monorepo warns
        # about: a report's hardware is the hardware MEASURED, not the rig that
        # happens to host the file. This block arrived from the G5g fork still
        # saying "NVIDIA T4G", "AWS Graviton2" and 15.36 GB, on a report
        # describing an L4. Corrected 2026-08-29.
        "hardware": {
            "accelerator": "NVIDIA L4",
            "chips": 1,
            "topology": "single device",
            # 23034 MiB as reported by nvidia-smi, not the nominal 24 GB.
            "hbm_gb_per_chip": 23.034,
            "machine_type": args.instance_type,
            "host": {
                "cpu": "AMD EPYC (x86_64)",
                "vcpus": args.vcpus,
                "ram_gb": args.host_ram_gb,
            },

...
Run on the G6 instance with the interpreter that holds the DLAMI's torch (the
bootstrap records it in /opt/torch-g6/PYTHON_BIN):
...
def resolve_compute_dtype(device: torch.device) -> torch.dtype:
    ...
    The L4 in this box is Ada (SM 8.9), which HAS a native bfloat16 datapath, so
    this resolves to bfloat16 and the dtype tax the Turing siblings pay is simply
    absent. Keep the probe anyway -- the branch is what makes that a measured
    fact rather than an assumption, and it is what lets the same file serve the
    T4G sibling correctly.

    The failure it guards against is silent in exactly one direction: **bfloat16
    on a pre-Ampere GPU does not raise.** CUDA accepts it and emulates through
    fp32, and the only symptom is that most of decode disappears into conversion.
    Turing (SM 7.5) has no bf16 datapath and no fp8; float16 is its only real
    16-bit path. So the dtype is read off the live device rather than taken on
    trust, and the resolved value is logged at startup.
    """
    import torch
...
def load_hf_token() -> None:
    ...
    a G6, so that path could only ever have failed here, silently, and left
...
class Metrics:
    ...
    def __init__(self, model_id: str, engine: KVCacheDecodeEngine) -> None:
        self.model_id = model_id
        self.engine = engine
        self.rig = os.getenv("RIG_NAME", "gpu-pytorch-g6-2b")

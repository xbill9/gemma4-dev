...
"""One-shot generate against the local checkpoint, for smoke-testing the box.
    ...
    python3 torch_generate.py --prompt "Explain Ada Lovelace GPUs in one sentence."
...
def resolve_compute_dtype(device):
    ...
    On this rig's L4 (Ada, SM 8.9) this resolves to bfloat16, which the device
    supports natively. The guard still matters because the failure is silent the
    other way: Turing (SM 7.5) has no bf16 datapath, and asking for bfloat16
    there does not raise -- CUDA emulates through fp32 and you lose most of
    decode. Same guard as `torch_openai_server.py`; kept in both because either
    can be run alone.
    """
    import torch
...
def main() -> None:
    ...
    p.add_argument("--prompt", default="Explain Ada Lovelace GPUs in one sentence.")
    ...
    if not torch.cuda.is_available():
        raise SystemExit(
            "no CUDA device. On this rig that usually means the resolved AMI "
            "carries no NVIDIA driver -- AWS also ships CPU-inference DLAMIs, "
            "which boot fine and simply have no GPU. Run `verify_gpu_arch` first."
        )

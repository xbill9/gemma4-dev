...
if [ -f "$SCRIPT_DIR/.claude/skills/tpu-pytorch-inf2-2b-management/SKILL.md" ]; then
  SKILL_SRC="$SCRIPT_DIR/.claude/skills/tpu-pytorch-inf2-2b-management"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "                           prefixes every tool as mcp__<name>__find_inf2."
      echo "                           (default: the rig directory, tpu-pytorch-inf2-2b)"
...
# The client's key for this server prefixes every tool (mcp__<key>__find_inf2), so it has to
# identify the rig. A shared constant is what made the sibling rigs indistinguishable in
...
if [ "$GLOBAL" -eq 1 ]; then
  SKILL_DEST="$HOME/.claude/skills/tpu-pytorch-inf2-2b-management"
else
  SKILL_DEST="$TARGET_DIR/.claude/skills/tpu-pytorch-inf2-2b-management"
fi
...
data.setdefault("mcpServers", {})[name] = {
    "command": "python3",
    "args": [".claude/skills/tpu-pytorch-inf2-2b-management/mcp/server.py"],
    "env": json.loads(env),
}

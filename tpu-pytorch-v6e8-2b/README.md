...
`project-setup.sh` installs the `tpu-pytorch-v6e8-2b-management` skill **and** registers the `tpu-pytorch-v6e8-2b` MCP server for any project (idempotent — re-run to refresh):
...
## 📦 Installing the `tpu-pytorch-v6e8-2b-management` Skill

Claude Code auto-discovers any skill folder containing a `SKILL.md` in two places:

- **Project-level:** `<project>/.claude/skills/tpu-pytorch-v6e8-2b-management/` — available only in that project (this repo ships its own copy, so working inside this repo needs no install).
- **User-level:** `~/.claude/skills/tpu-pytorch-v6e8-2b-management/` — available in every project on the machine.
...
| One specific project (skill **and** `tpu-pytorch-v6e8-2b` MCP server) | `make init TARGET=/path/to/project ARGS='--project <gcp-project-id>'` |
| All projects + user-scope MCP registration | `make init ARGS='--global'` |
| Another machine | `make skill-package`, copy `dist/tpu-pytorch-v6e8-2b-management-skill.zip`, unzip into `~/.claude/skills/` |
...
/plugin install tpu-pytorch-v6e8-2b-management@tpu-skill-claude
```

This installs the `tpu-pytorch-v6e8-2b-management` skill **and** registers the `tpu-pytorch-v6e8-2b` MCP server in one step, with updates managed by Claude Code (`/plugin` → manage/update). Configure the server through environment variables (e.g. `GOOGLE_CLOUD_PROJECT`, `MODEL_NAME`, `ACCELERATOR_TYPE`) — see `SKILL.md` or the `get_help` tool for the full list.
...
./project-setup.sh --global                          # skill + user-scope tpu-pytorch-v6e8-2b MCP server
```

**Option C — zip install, no clone** (straight from the packaged zip):

```bash
curl -L -o /tmp/tpu-pytorch-v6e8-2b-management-skill.zip \
  https://github.com/xbill9/tpu-skill-claude/raw/main/dist/tpu-pytorch-v6e8-2b-management-skill.zip
mkdir -p ~/.claude/skills && unzip -o /tmp/tpu-pytorch-v6e8-2b-management-skill.zip -d ~/.claude/skills/
~/.claude/skills/tpu-pytorch-v6e8-2b-management/mcp/project-setup.sh --global   # optional: register the MCP server
```
...
After installing (or updating), **restart Claude Code** or start a new session so it picks up the skill. Verify with `/skills` — `tpu-pytorch-v6e8-2b-management` should be listed.
...
| Path | Purpose |
| :--- | :--- |
| `server.py` | The `tpu-pytorch-v6e8-2b` FastMCP server — the authoritative source (full tool catalog in `SKILL.md` / the `get_help` tool) |
...
| `.claude/skills/tpu-pytorch-v6e8-2b-management/` | Project-level skill: `SKILL.md`, `mcp/` (server snapshot, installer, startup script template), `references/tpu-guide.md` |
| `.claude-plugin/` | `plugin.json` + `marketplace.json` — the repo doubles as a Claude Code plugin marketplace |
| `skills/tpu-pytorch-v6e8-2b-management/` | Plugin-layout copy of the skill (synced by `make skill`) |
| `dist/tpu-pytorch-v6e8-2b-management-skill.zip` | Packaged skill for zip installs (built by `make skill-package`) |
...
The `tpu-pytorch-v6e8-2b` MCP server covers the full TPU serving lifecycle (catalog with usage guidance in [SKILL.md](.claude/skills/tpu-pytorch-v6e8-2b-management/SKILL.md), live listing via the `get_help` tool):
...
make skill-package # Refresh + build dist/tpu-pytorch-v6e8-2b-management-skill.zip
make init TARGET=/path/to/project [ARGS='--project my-gcp-id']
                   # Refresh + install skill AND register the tpu-pytorch-v6e8-2b MCP server
```
...
- [SKILL.md](.claude/skills/tpu-pytorch-v6e8-2b-management/SKILL.md) — the skill itself: lifecycle, tool catalog, required vLLM flags, field notes, cautions

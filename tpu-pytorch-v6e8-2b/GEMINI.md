# 🤖 Gemini Workspace Context: TPU Management Skill & tpu-pytorch-v6e8-2b MCP Agent
...
This repository packages a Claude Code skill (`tpu-pytorch-v6e8-2b-management`) and a **Model Context Protocol (MCP) server** (`tpu-pytorch-v6e8-2b`) that together act as an AI DevOps/SRE agent for Google Cloud TPUs. Two main purposes:
...
- **MCP server source:** [server.py](server.py) — the authoritative `tpu-pytorch-v6e8-2b` FastMCP agent (full tool catalog in `SKILL.md` / the `get_help` tool)
- **Skill definition:** [.claude/skills/tpu-pytorch-v6e8-2b-management/SKILL.md](.claude/skills/tpu-pytorch-v6e8-2b-management/SKILL.md) — lifecycle, tool catalog, required vLLM flags, field notes
...
- **Reference guide:** `.claude/skills/tpu-pytorch-v6e8-2b-management/references/tpu-guide.md` — TPU getting started guide: zones, quotas, troubleshooting
...
make skill-package # ...and rebuild dist/tpu-pytorch-v6e8-2b-management-skill.zip
```

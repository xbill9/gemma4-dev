...
# Tensor parallel degree. 8 uses the whole slice and matches the twin rig tpu-vllm-v6e8-2b.
#
# IT IS NOT A FREE WIN FOR THIS CHECKPOINT. ../MODELS.md records E2B as full MQA with
# `num_key_value_heads=1`. The KV cache shards across KV heads, not weight columns, so a
# layer's cache shards min(TP, num_kv_heads) ways — 1 at every TP. At TP=8 the runtime pads
# the head count up to the parallel degree and E2B's KV cost is multiplied by 8 rather than
# divided, while the weights that do shard are only ~4.5 GiB per chip to start with. The
# slice buys aggregate HBM and batch headroom, not a shorter decode step. See tpu.env and
# When_TP_Crosses_the_KV_Head_Count_v6e8.pdf, which measures the same crossing on 31B.

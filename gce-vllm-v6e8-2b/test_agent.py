...
class TestNodeDiscovery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_ct6e_instance_is_not_a_tpu_vm_node(self):
        ...
        async def _run(cmd, timeout=60):
            ...
            if "queued-resources" in cmd:
                return 0, "[]", ""
            if "instances" in cmd and "list" in cmd:
                return 0, json.dumps([_node("gce-vllm-v6e8-2b")]), ""
            return 1, "", "unexpected"
    ...
    async def test_no_remote_tool_reaches_the_vm_through_tpu_vm_ssh(self):
        ...
        with (
            patch("server.run_command", new=AsyncMock(side_effect=_run)),
            patch("server._resolve_node_id", new=AsyncMock(return_value="gce-vllm-v6e8-2b")),
        ):
            await server.manage_vllm_docker("gce-vllm-v6e8-2b", action="status")
            await server.get_vllm_docker_logs("gce-vllm-v6e8-2b")
            await server.get_tpu_system_logs("gce-vllm-v6e8-2b")
    ...
    async def test_docker_commands_install_docker_when_the_image_lacks_it(self):
        ...
        with (
            ...
            await server.manage_vllm_docker("gce-vllm-v6e8-2b", action="start")
            docker_cmd = seen[-1][-1]
            seen.clear()
            await server.get_tpu_system_logs("gce-vllm-v6e8-2b")

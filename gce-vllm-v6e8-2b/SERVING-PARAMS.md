...
> **This rig moved from v6e-1 to v6e-8 on 2026-08-19, which demotes the `v6e` tag below.** Every row
> tagged **v6e** is grounded in a *single-chip* v6e measurement. Per-chip properties (HBM, bandwidth,
> native formats, the fp8-KV null result) still hold — they are chip facts. Anything that is a
> *deployment* property — KV pool size, the memory-utilization ceiling, where the batch knee sits —
> now describes a slice this rig no longer provisions. Nothing here has been measured at eight chips.
...
| `--tensor-parallel-size` | **8** | **v6e-8 — untested, and the weakest row here** | Set to 8 to use the slice and to match the twin `tpu-vllm-v6e8-2b`. **The model argues against it**: E2B is full MQA with `num_key_value_heads=1`, its cache shards `min(TP, 1) = 1` way at any degree, so TP=8 pads heads up and multiplies per-token KV cost by 8 rather than dividing it (`../MODELS.md`). The weights do shard, but they are only ~4.5 GiB/chip. `When_TP_Crosses_the_KV_Head_Count_v6e8.pdf` measures the same crossing on 31B on this machine type: 1.92x per layer where KV heads still shard, 1.26x where they do not. **Sweep TP=1/2/4/8 before treating 8 as settled** |

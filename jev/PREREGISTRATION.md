...
- **Deviation in run `2026-09-24-l4-suite`:** the E4B arm shared result files with the 26B plain arm (both `autoregressive-<subset>.jsonl` in one run), so the resume logic skipped every E4B record. E4B was re-run on the same instance, image, flags and records under run `2026-09-24-l4-suite-e4b`. `run_suite.py` now refuses a run directory that holds another model's results.

#!/usr/bin/env bash
# Install the bundled skill and register its EC2 G4dn MCP server.
set -euo pipefail
...
# DERIVED, never a literal. A hardcoded stem is what silently survives a rename:
# on the JAX rig's G5g -> G6 fork this line still read the OLD rig's name, so the
# script could not find the skill at all and died with "cannot locate the bundled
# skill" -- the rig was UNREGISTERABLE, not merely misregistered. The Makefile and
# refresh_skill.py both derive it from the rig directory; this must agree.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --instance-type TYPE     g4dn.xlarge, g4dn.2xlarge, g4dn.4xlarge,"
      echo "                           g4dn.8xlarge, g4dn.12xlarge, g4dn.16xlarge, g4dn.metal"
      echo "                           (g4dn.xlarge is ACCEPTED here: G4dn has 16 GiB of host"
      echo "                            RAM at xlarge, twice its g5g namesake)"
      echo "  --server-name NAME       MCP server name; the key it is registered under, which"
      echo "                           prefixes every tool as mcp__<name>__create_g4dn_instance."
      echo "                           (default: the rig directory, gpu-vllm-g4dn-2b)"
...
# Every G4dn size is accepted. The G5g rig rejected g5g.xlarge for having 8 GiB
# of host RAM; g4dn.xlarge has 16, so that rejection does not carry -- and the
# remedy for a small host is a swapfile anyway, not a bigger instance.
#
# Keep this list in step with _G4DN_SIZES in server.py. Note g4dn.metal rather
# than a .24xlarge/.48xlarge tail: the G4dn family is shaped differently from G6.
